...
<c:if test="${'1' eq sessionScope.userInfo.authorId or '2' eq sessionScope.userInfo.authorId}">
	<c:if test="${not empty quickMenuList}">
		<c:forEach var="i" items="${quickMenuList }" varStatus="status">
		...
		<c:if test="${cnt > 0 }" >
			<div class="wrap_quickMenu">
			   ...
			   <div class="area_cont">
			     <ul>
			     	<c:forEach var="i" items="${quickMenuList }" varStatus="status">
			     		<c:if test="${i.ICON_NM != '' and i.LINK != '' and i.IMG_PATH != '' and i.IMG_NM != '' and i.OPEN_YN == 'Y'}">
					       <li>
					         <div class="ico">
					           <a href="#" onclick="move('${i.LINK}'); return false;">
					           	<img src="${i.IMG_PATH}/${i.IMG_NM}" alt="이미지">
					           </a>
					         </div>
					         <div class="txt">
					          <a href="#" onclick="move('${i.LINK}'); return false;">${i.ICON_NM}</a>
					         </div>
					       </li>
				       </c:if>
			       </c:forEach>
