...
<script type="text/javascript">
	...
	$(window).ready(function(){
	    ...
	    $('#createIndex').unbind('click').bind('click',function(){
	    	...
	    	$("#indexListForm").attr({
                action : "/admin/index/createMngLevelIndex.do",
                method : "post"
            }).submit();
	    });
	...
	function fn_modify(lclas) {
    	...
    	$("#indexListForm").attr({
            action : "/admin/index/modifyMngLevelIndex.do",
            method : "post"
        }).submit();
	}
	...
	function fn_excel_download() {
    	$("#indexListForm").attr({
            action : "/admin/index/mIndexExcelDownload.do",
            method : "post"
        }).submit();
	}
	...
	function fn_excel_upload(){
		...
		if (confirm("등록된 지표는 삭제됩니다. 엑셀 업로드 하시겠습니까?")) {
			...
			$("#indexListForm").attr("action", "/admin/index/mIndexExcelUpload.do");
...
<form action="/admin/index/mngLevelIndexList.do" method="post" id="indexListForm" name="indexListForm" enctype="multipart/form-data">
	
	<input name="lclas" id="lclas" type="hidden" value=""/>
	<input name="mngLevelCd" id="mngLevelCd" type="hidden" value="ML01"/>
	        ...
	        <%-- <c:if test="${resultCheck != 'Y'}"> --%>
		        <div class="body" style="min-height: auto;">
		            ...
		            <div class="board_list_btn right">
		                <a href="nomalInfoeWrite.jsp" class="btn blue" onclick="javascript:fn_default(); return false;">기본정보 수정</a>
		            </div>
		        </div>
		    <%-- </c:if> --%>
	    </div>
					...
					<c:when test="${resultCheck != 'Y'}">
		                <c:choose>
							<c:when test="${!empty resultList}">
					            <table class="board_list_write" summary="분야, 진단지표, 진단항목, 상세·수정으로 구성된 지표등록 리스트입니다.">
					                <thead>
					                    <tr>
					                        <th style="width: 220px;">분야</th>
					                        <th>진단지표</th>
					                        <th>진단항목</th>
				                        	...
				                        	<c:choose>
												<c:when test="${status.first}">
													<tr>
						                                ...
						                                <td rowspan="${list.lclasCnt}" class="tc" style="text-align: center;">
					            ...
					            <c:if test="${'Y' eq addFlag && 'Y' eq modifyFlag}">
	            					<div class="board_list_btn right">
					            		<!-- <a href="indicatorWrite.jsp" class="btn blue" id="createIndex">지표 등록</a> -->
