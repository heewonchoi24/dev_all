						...
						<c:forEach var="i" items="${bbsList }" varStatus="status">
							...
							<c:choose>
								<c:when test="${((bbsCd == 'BN05' or bbsCd == 'BN07') and sessionScope.userInfo.authorId eq '7')}">
									<a href="#" onclick="modify('${i.SEQ}'); return false;" class="t_gall1">
								</c:when>
								<c:otherwise>
									<a href="#" onclick="detail('${i.SEQ}' , '${i.REGIST_ID }'); return false;" title="상세보기" class="t_gall1">
								</c:otherwise>
							</c:choose>
								<div class="photo">
									<span class="i-cut">
								...
								<div class="cont">
									<c:set var="subject" value='${fn:substring(i.SUBJECT.replaceAll("\\\<.*?\\\>",""), 0, 50) }'/>
									<p class="h1"><c:if test="${bbsCd == 'BN01' && i.HEAD_CATEGORY_TEXT != '' }">[${i.HEAD_CATEGORY_TEXT }] </c:if><c:out value='${subject.replaceAll("  ", "") }' escapeXml="false"/></p>
									<p class="t1"><span><em>등록일</em>${i.REGIST_DT }</span><span><em>조회수</em>${i.READ_COUNT }</span></p>
								</div>
								</a>
							</li>
						</c:forEach>
