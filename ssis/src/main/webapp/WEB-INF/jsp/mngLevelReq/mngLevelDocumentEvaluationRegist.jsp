...
<style> .icheckbox_square + label { width: 95%; } input[type='text'].inp_txt, input[type='password'].inp_txt, input[type='text']:disabled{ line-height: 0; } .i-aft.link a { text-decoration: underline; color: #0d6090; font-size: 13px; }</style>
...
$(document).ready(function(){
	...
	$(".lst_all_chk .iCheck-helper, .lst_all_chk label").on("click", function() {
...
function fnAttachmentFileCallback( uploadedFilesInfo, modifiedFilesInfo ) {
	...
	if($("#gubun").val() == "FobjctFile"){
		fnAttachmentFobjctFileCallback( uploadedFilesInfo, modifiedFilesInfo );
		return true;
	}
...
function fnAttachmentFobjctFileCallback( uploadedFilesInfo, modifiedFilesInfo ) {
	...
	pParam.atchmnflId = $("#atchmnfl_id").val();
	pParam.uploadedFilesInfo = uploadedFilesInfo;
	pParam.modifiedFilesInfo = modifiedFilesInfo;
	
	pParam.insttCd    = $("#s_instt_cd").val();
	pParam.indexSeq   = $("#s_index_seq").val();
...
function fn_save() {
	...
	// 평가 결과 점수 유무 체크
	var resultScore = '${mngLevelInsttDetailEvlInfo.RESULT_SCORE2 }';
	if(resultScore == ''){
		alert("평가 점수가 등록되어야 이의신청이 가능합니다.");
		$("#fobjctResn").text("");
		return true;
	}
	...
	$.ccmsvc.ajaxSyncRequestPost(pUrl, param, function(data, textStatus, jqXHR){
		...
		$("#form").attr({
            action : "/mngLevelReq/mngLevelDocumentEvaluationRegist.do",
            method : "post"
        }).submit();
...
function fnAddAttachmentFobjctFile(index, atchmnflId, attachmentFile){
	// WRITE 권한 체크
	if(authCheckWrite() == false) return false;
	
	$( "#atchmnfl_id" ).val(atchmnflId);
    $( "#indexSeq" ).val(index);
    $( "#gubun" ).val("FobjctFile");
...
<form method="post" id="form" name="form" >
	...
	<!-- 이의신청 -->
	<input type="hidden" name="insttCd"  value="${s_instt_cd}" />
	<input type="hidden" name="indexSeq" value="${s_index_seq}" />
	<input type="hidden" name="orderNo"  value="${s_order_no}" />
	<!--// 이의신청 -->
                ...
                <table id="table-1" class="tbl">
                    ...
                    <tbody>
	                            ...
	                            <td colspan="3" class="pd0">
									<div class="lst_tblinner">
										<div class="tbls">
												...
												<div class="tbls">
													<div class="col list">
														...
														<div class="lst_all_chk" id="lst_all_chk1">
				                                        	...
				                                        	<label for="chkAttachmentFile_${s_index_seq}_evl" id="lst_all_chk2">전체선택</label>
											...
											<div class="col btns">
												<c:choose>
				                                	<c:when test="${mngLevelInsttDetailEvlInfo.EXCP_PERM_YN == 'Y' and sessionScope.userInfo.authorId ne '2'}">
				                                		<input type="text" id="result_score" class="inp_txt w100p" value="${mngLevelInsttDetailEvlInfo.RESULT_SCORE2 }" disabled="disabled" title="평가결과 입력란">
				                                 	</c:when>
				                                	<c:when test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId ne '2'}">
						...
						<tr>
				            ...
				            <c:choose>
								<c:when test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId ne '2'}">
									<input id="evl_opinion" maxlength="1000" type="text" class="inp_txt w100p ty2" value="${mngLevelInsttDetailEvlInfo.EVL_OPINION2 }" placeholder="진단항목에 대한 평가의견이 있을 경우 입력해주세요."  title="평가의견 입력란">
								 </c:when>
								<c:otherwise>
						...
						<c:if test="${sessionScope.userInfo.authorId ne '2'}">
							<tr>
								...
								<td headers="a_c" colspan="4" style="text-align: left;">
									<c:choose>
										<c:when test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId ne '2'}">
											<input type="text" id="memo" class="inp_txt w100p ty2"  maxlength="1000" value="${mngLevelInsttDetailEvlInfo.MEMO }" placeholder="특이사항이나 이슈사항이 있을 경우 입력해주세요." title="메모 입력란">									
										</c:when>
										<c:otherwise>
 						...
 						<tr>
							<th scope="col" id="a_d" class="bdl0">평가 </br>첨부파일</th>
							<td headers="a_d" colspan="4" class="ta-l"> 
								<c:if test="${( fn:contains(periodCode, 'C') ||  fn:contains(periodCode, 'B') ) and sessionScope.userInfo.authorId ne '2'}">
									<a id="btn_${mngLevelInsttDetailEvlInfo.INDEX_SEQ}" class="btn-pk vs blue" onclick="fnAddAttachmentFile('${mngLevelInsttDetailEvlInfo.INDEX_SEQ}','${mngLevelInsttDetailEvlInfo.ATCHMNFL_ID}','attachmentFile'); return false;"><span>파일찾기</span></a>
								</c:if>
								<c:if test="${! empty mngLevelInsttDetailMemoFileList }">
									...
									<div class="lst_all_chk solo" id="lst_all_chk2" style="padding-left: 0;">
                                      	<input class="ickjs" type="checkbox" id="chkAttachmentFile_${s_index_seq}_memo" value="" onClick="javascript:fnCheckAttachmentMemoFile('${s_index_seq}');">
                                       	<label for="chkAttachmentFile_${s_index_seq}_memo" id="lst_all_chk2">전체선택</label>
                                       	
										<c:if test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId ne '2'}">
											<a class="btn-pk ss purple rv fl-r ml10" onClick="javascript:fnDeleteAttachmentFile('${s_index_seq}','','attachmentFile'); return false;" style="margin-top: 5px;"><span>파일삭제</span></a>
										</c:if>                                     	
						...
						<th scope="col" id="a_c" class="bdl0">이의신청</th>
							<td headers="a_c" colspan="4" style="text-align: left;">
	                           	  <c:choose>
	                            	<c:when test="${('C' eq requestZvl.periodCd || 'B' eq requestZvl.periodCd) and sessionScope.userInfo.authorId eq '2'}">
	                            		<input type="text" id="fobjctResn" name="fobjctResn" class="inp_txt w100p ty2" placeholder="이의신청 사유를 입력해주세요." title="이의신청 입력란" value="${mngLevelFobjctResn.FOBJCT_RESN}">
	                            	</c:when>
	                            	<c:otherwise>
	                            		${mngLevelFobjctResn.FOBJCT_RESN}
	                            	</c:otherwise>
	                              </c:choose>		
					        </td>
			            </tr>
<%--  						<tr>
							<th scope="col" id="a_d" class="bdl0">이의신청 </br>첨부파일</th>
							<td headers="a_d" colspan="4" class="ta-l"> 
								<c:if test="${( fn:contains(periodCode, 'C') ||  fn:contains(periodCode, 'B') ) and sessionScope.userInfo.authorId eq '2'}">
									<a id="btn_${mngLevelInsttDetailEvlInfo.INDEX_SEQ}" class="btn-pk vs blue" onclick="fnAddAttachmentFobjctFile('${mngLevelInsttDetailEvlInfo.INDEX_SEQ}','${i.fileId }','attachmentFile'); return false;"><span>파일찾기</span></a>
								</c:if>
								<c:if test="${! empty fobjctFile }">
									<div class="lst_answer solo" id="lst_answer2" style="border-top: none; padding-left: 0;">
										<c:forEach var="i" items="${fobjctFile }" varStatus="status">
											<div>
												<input type="checkbox" class="ickjs" id="${i.fileId }" name="chkAttachmentFile_${i.indexSeq }_memo">
												<label for="${i.fileId }"><span class="i-aft i_${i.fileExtsn } link"><a href="#" onclick="fnAttachmentFobjctFileDown('${result.indexSeq}','${i.fileId }'); return false;" class="f_<c:out value='${i.fileExtsn}'/>">${i.fileName }</a></span></label>
											</div>
											<script> 
			                                var fileInfo = new Object();
			                                fileInfo.idx			= '${i.indexSeq}';
			                                fileInfo.atchmnflId		= '${i.fobjctAtchmnflId}';
									...
									<div class="lst_all_chk solo" id="lst_all_chk2" style="padding-left: 0;">
                                      	<input class="ickjs" type="checkbox" id="chkAttachmentFile_${s_index_seq}_fobjct" value="" onClick="javascript:fnCheckAttachmentFobjctFile('${s_index_seq}');">
                                       	<label for="chkAttachmentFile_${s_index_seq}_fobjct" id="lst_all_chk2">전체선택</label>
                                       	
										<c:if test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId eq '2'}">
											<a class="btn-pk ss purple rv fl-r ml10" onClick="javascript:fnDeleteAttachmentFile('${s_index_seq}','','attachmentFile'); return false;" style="margin-top: 5px;"><span>파일삭제</span></a>
										</c:if>                                     	
                                      	
										<a class="btn-pk ss gray2 rv fl-r" onClick="javascript:fnAttachmentFobjctFileDown('${s_index_seq}',''); return false;" style="margin-top: 5px;"><span>다운로드</span></a>
									</div>			
								</c:if>							
							</td>
						</tr>  --%>				            
            ...
            <div class="btn-bot noline ta-c">
            	<c:if test="${(fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')) and sessionScope.userInfo.authorId ne '2' and requestZvl.s_order_no eq currentOrderNo}">
					<a href="#" onclick="done(); return false;" class="btn-pk n purple rv">평가완료</a>
				</c:if>
            	<c:if test="${'2' eq sessionScope.userInfo.authorId and ('C' eq requestZvl.periodCd || 'B' eq requestZvl.periodCd)}">
					<a href="#" onclick="fn_save(); return false;"  class="btn-pk n purple rv">저장</a>
				</c:if>				
                <a href="#" onclick="goBack(); return false;" class="btn-pk n black">목록으로</a>
            </div>            
        </div>
    </section>
    <!-- /content -->
