...
<style> .icheckbox_square + label { width: 95%; } </style>
...
<!--        
            ...
            <div class="wrap_table2">
                <table id="table-1" class="tbl">
                    <caption>분야, 진단지표, 진단항목, 평가점수, 환산점수로 구성된 이의신청</caption>
                    <colgroup>
			            <col class="th1_1">
			            <col class="th1_2">
			            <col>
                    ...
                    <thead>
			            <tr>
			              ...
			              <th scope="col" id="th_d">평가점수</th>
			              <th scope="col" id="th_e">환산점수</th>
			            </tr>
                    </thead>
                    <tbody>
                        <tr>
	                            ...
	                            <td colspan="3" class="pd0">
									<div class="lst_tblinner">
										<div class="tbls">
											...
											<div class="col state">
												<c:if test="${not empty mngLevelInsttDetailList }">
													<c:forEach var="i" items="${mngLevelInsttDetailList }" begin='0' end='1'>
														${i.RESULT_TOT_PER1 }
													</c:forEach>
												</c:if>											
											</div>
	                            ...
	                            <td colspan="3" class="pd0">
									<div class="lst_tblinner">
										<div class="tbls">
											...
											<div class="col state">
												<c:if test="${empty mngLevelInsttDetailList }">
													<c:forEach var="i" items="${mngLevelInsttDetailList }" begin='0' end='1'>
														${i.RESULT_SCORE1 }
													</c:forEach>
												</c:if>
 						...
 						<tr>
							...
							<td headers="a_d" colspan="4" class="ta-l"> 
								<%-- <c:if test="${fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')}">
									<a style="margin: 0 0px 11px 20px;" id="btn_${mngLevelInsttDetailEvlInfo.INDEX_SEQ}" class="btn-pk vs blue" onclick="fnAddAttachmentFile('${mngLevelInsttDetailEvlInfo.INDEX_SEQ}','${mngLevelInsttDetailEvlInfo.ATCHMNFL_ID}','attachmentFile'); return false;"><span>파일찾기</span></a>
								</c:if> --%>
								<c:if test="${! empty mngLevelInsttDetailMemoFileList }">
									...
									<%-- 	<c:if test="${fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')}">
											<a type="button" class="btn-pk ss purple rv fl-r ml10" onClick="javascript:fnDeleteAttachmentFile('${s_index_seq}','','attachmentFile'); return false;"><span>파일삭제</span></a>
										</c:if>  --%>                                    	
                                      	
										<a type="button" class="btn-pk ss gray2 rv fl-r" onClick="javascript:fnAttachmentMemoFileDown('${s_index_seq}',''); return false;" style="margin-top: 5px;"><span>다운로드</span></a>
									</div>			
								</c:if>							
							</td>
						</tr> 
...
<%--             	<c:if test="${fn:contains(periodCode, 'B') ||  fn:contains(periodCode, 'D')}">
					<a href="#" onclick="done(); return false;" class="btn-pk n purple rv">신청완료</a>
				</c:if> --%>
