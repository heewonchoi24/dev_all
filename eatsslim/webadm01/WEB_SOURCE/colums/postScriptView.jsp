...
if (request.getParameter("id") != null && request.getParameter("id").length() > 0) {
	...
	//댓글수
	query		= "SELECT COUNT(ID) FROM ESL_PO_REPLY where ID="+noticeId+"";
	...
	query		= "SELECT TITLE, CONTENT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE, PRESS_URL,HIT_CNT,INST_ID,LIST_IMG ";
	query		+= " FROM "+ table;
	query		+= " WHERE ID = ?";
...
<script src="postScriptView.js"></script>
				   ...
				   <div class="post-wrapper">
				      <div class="post-read">
						  ...
						  <div class="post-contents" >
							  <% if(!imgUrl.equals("")){ %>
									<!-- <div style="text-align:center;width:700px;"><img src="<%=imgUrl%>"></div><br /><br /> -->
							  <%} %>
						      <%=content%>
                            <!-- 이전/다음 컬럼 보기 -->
                            <!-- <div class="viewNavi">
						  ...
						  <div class="col center">
							<div class="button small dark"><a href="/colums/postScript.jsp?<%=param%>">목록</a></div>
							<% if(inst_id.equals(eslMemberId)){%>
								<div class="button small dark"><a href="#" onclick="window.open('/colums/postWritePopup.jsp?id=<%=noticeId + param%>','PostWrite','width=1024,height=600,scrollbars=1')">수정</a></div>
								<div class="button small dark"><a href="javascript:board_id_del('<%=noticeId%>');">삭제</a></div>
							<% } %>
						</div>
						  <div class="comment-wrap">
							  ...
							  <%
									...
									if(reply_idx > 0){
										query		= "SELECT * ";
										query		+= " FROM ESL_PO_REPLY where ID="+noticeId+" and IDX="+reply_idx+" and M_ID='"+eslMemberId+"'";
						      ...
						      <ul>
							    <li class="comment depth-1" >
									...
									<%
									query		= "SELECT * ";
									query		+= " FROM ESL_PO_REPLY where ID="+noticeId+"";
									query		+= " ORDER BY IDX ASC";
								   ...
								   <div class="commentheader">
								      ...
								      <a href="/colums/postScriptView.jsp?<%=param%>&id=<%=noticeId%>&reply_idx=<%=reply_idx%>">수정</a> ㅣ <a href="javascript:reply_del('<%=reply_idx%>');">삭제</a>
									  <% } %>
									   </div>
								   </div>
		...
		<div class="sidebar four columns">
		   <div class="colums-side">
              <ul class="listNavi">
                  ...
                  <li class="active"><a href="/colums/postScript.jsp?<%=param%>"><span class="current"></span>목록</a></li>
                  <li class="last">
              ...
              <ul class="recent">
				<%
				query		= "SELECT ID, TITLE, LIST_IMG, PRESS_URL, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE,CONTENT,HIT_CNT,(select count(id) from ESL_PO_REPLY where ID="+table+".ID) as re_cnt";
				if(p_gubun.equals("")){
					query		+= " FROM "+ table + "";
				}else{
