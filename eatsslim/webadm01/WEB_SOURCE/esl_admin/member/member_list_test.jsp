	...
	<div id="container" class="group">
		...
		<div id="incon">
			...
			<div id="contents">
								...
								<th scope="row">
									<span>
										<select style="width:80px;" name="field" onchange="this.form.keyword.focus()">
											<option value="MEM_ID"<%if(field.equals("TITLE")){out.print(" selected=\"selected\"");}%>>아이디</option>
											<option value="MEM_NAME"<%if(field.equals("CONTENT")){out.print(" selected=\"selected\"");}%>>회원명</option>
										</select>
									</span>
								</th>
				...
				<form name="frm_list" id="frm_list" method="post" action="notice_db.jsp" enctype="multipart/form-data">
						...
						<tbody>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									//쿠폰내역
									query="SELECT COUPON_NUM, COUPON_NAME, USE_YN, USE_DATE FROM ESL_COUPON C, ESL_COUPON_MEMBER CM WHERE C.ID = CM.MEMBER_ID AND CM.MEMBER_ID = '"+rs.getString("MEM_ID")+"'";
									...
									if (rs1.next()) {
										COUPON_NAME		= rs1.getString("COUPON_NAME");
									}else{
										COUPON_NAME = "";
									}	
				...
				<!-- <div class="btn_style1">
					...
					<p class="right_btn">
						<a href="notice_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div> -->
