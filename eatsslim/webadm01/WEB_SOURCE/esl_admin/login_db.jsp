...
if (mode.equals("login")) {
	query			= "SELECT ID, ADMIN_ID, ADMIN_NAME, ADMIN_MENU, DATEDIFF(NOW(), PW_UPDATE_DATE) AS PW_MDATE FROM "+ table +" WHERE ADMIN_ID = ? AND ADMIN_PW = PASSWORD(?)";
	...
	if (rs.next()) {
		...
		try {
			...
			if (pwModDate > 180 ) {
				ut.jsAlert(out, "비밀번호를 변경하신지 6개월이 넘었습니다.\\n비밀번호를 변경해 주세요.");
			}
