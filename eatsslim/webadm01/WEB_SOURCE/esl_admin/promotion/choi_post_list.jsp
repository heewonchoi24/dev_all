...
query		= "SELECT ID, TITLE, HIT_CNT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE,CATEGORY,(SELECT COUNT(ID) FROM "+ table +"_COMMENT WHERE CP_ID="+table+".ID) AS RE_CNT";
	...
	<div id="container" class="group">
		...
		<div id="incon">
			...
			<div id="contents">
					...
					<table class="table01" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<td>
									<select style="width:140px;" name="post_type">
										<option value="">선택</option>
										<option value="1"<%if(postType.equals("1")){out.print(" selected=\"selected\"");}%>>식사다이어트</option>
										<option value="2"<%if(postType.equals("2")){out.print(" selected=\"selected\"");}%>>프로그램다이어트</option>
										<option value="3"<%if(postType.equals("3")){out.print(" selected=\"selected\"");}%>>타입별다이어트</option>
									</select>
								</td>
								<th scope="row">
									<span>
				...
				<form name="frm_list" id="frm_list" method="post" action="choi_post_db.jsp" enctype="multipart/form-data">
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							...
							%>
							<tr>
								<td><input type="checkbox" class="selectable" value="<%=postId%>" /></td>
								<td><%=curNum%></td>
								<td><%=ut.getPostType(category)%></td>
								<td><%=ut.cutString(120, title, "..")%></td>
								<td><%=hitCnt%></td>
								<td><%=rs.getInt("re_cnt")%></td>
								<td><%=instDate%></td>
								<td>
									<a href="choi_post_edit.jsp?id=<%=postId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
				...
				<div class="btn_style1">
					...
					<p class="right_btn">
						<a href="choi_post_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div>
