...
query		= "SELECT ID, GUBUN, EVENT_TYPE, TITLE, OPEN_YN, STDATE, LTDATE, HIT_CNT";
						...
						<tbody>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									query1		= "SELECT COUNT(ID) FROM ESL_EVENT_REPLY WHERE ID = "+ eventId +" ORDER BY ID DESC";
									try {
							...
							<tr>
								...
								<td><a href="event_join_view.jsp?id=<%=eventId + param%>"><%=ut.cutString(60, title, "..")%></a></td>
								<td><%=stdate%>~<%=ltdate%></td>
								<td><%=hitCnt%></td>
								<td><%=commentCnt%></td>
							</tr>
