...
String comment		= ut.inject(multi.getParameter("comment"));
int reply_idx		= 0;
...
if (mode != null) {
	if (mode.equals("write")) {
		...
		out.print("<script>alert('등록되었습니다');location.href='choi_post_edit.jsp?"+param+"';</script>");
		...
		try {
			...
			pstmt.setString(2, "eatsslim");
			pstmt.setString(3, "잇슬림");
		...
		try {
			query		= "DELETE FROM "+ table +" WHERE ID IN ("+ reply_idx +") ";
		...
		out.print("<script>alert('삭제되었습니다');location.href='choi_post_edit.jsp?"+param+"';</script>");
