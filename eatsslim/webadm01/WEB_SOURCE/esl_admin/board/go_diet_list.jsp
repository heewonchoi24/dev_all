...
query		= "SELECT ID, TOP_YN, TITLE, HIT_CNT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE";
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<th scope="col"><span>상단공지</span></th>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									topYn		= (rs.getString("TOP_YN").equals("Y"))? "상단공지" : "-";
									title		= rs.getString("TITLE");
									hitCnt		= rs.getInt("HIT_CNT");
									instDate	= rs.getString("WDATE");
							%>
							<tr>
								<td><input type="checkbox" class="selectable" value="<%=noticeId%>" /></td>
								<td><%=curNum%></td>
								<td><%=topYn%></td>
								...
								<td><%=instDate%></td>
								<td>
									<a href="notice_edit.jsp?id=<%=noticeId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
