...
query		= "SELECT ID, TITLE, HIT_CNT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE,PRESS_URL,(select count(id) from ESL_PO_REPLY where ID="+table+".ID) as re_cnt";
	...
	<div id="container" class="group">
		...
		<div id="incon">
			...
			<div id="contents">
							...
							<tr>
								...
								<td>
									<select style="width:140px;" name="post_type">
										...
										<option value="4"<%if(postType.equals("4")){out.print(" selected=\"selected\"");}%>>이벤트후기</option>
				...
				<form name="frm_list" id="frm_list" method="post" action="po_db.jsp" enctype="multipart/form-data">
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							...
							<tr>
								...
								<td><%=curNum%></td>
								<td><%=ut.getPostType(pressUrl)%></td>
								<td><%=ut.cutString(120, title, "..")%></td>
								<td><%=hitCnt%></td>
								<td><%=rs.getInt("re_cnt")%></td>
								<td><%=instDate%></td>
								<td>
									<a href="po_edit.jsp?id=<%=pressId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
				...
				<div class="btn_style1">
					...
					<p class="right_btn">
						<a href="po_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div>
