...
<%
	...
	headerQuery.append(" SELECT MEM_NAME, HP, EMAIL, SMS_YN, EMAIL_YN FROM ESL_MEMBER WHERE 1=1 AND mem_id = '"+ eslMemberId + "'" );
	headerStmt = conn.createStatement();
	headerRs = headerStmt.executeQuery(headerQuery.toString());

	while(headerRs.next() ){
		memHp = headerRs.getString("HP") == null ? "": headerRs.getString("HP");
		memName = headerRs.getString("MEM_NAME") == null ? "": headerRs.getString("MEM_NAME");
		memEmail = headerRs.getString("EMAIL") == null ? "": headerRs.getString("EMAIL");
		smsYn = headerRs.getString("SMS_YN") == null ? "": headerRs.getString("SMS_YN");
		emailYn = headerRs.getString("EMAIL_YN") == null ? "": headerRs.getString("EMAIL_YN");
	}
...
.cellw1 {width:30%;display:table-cell;vertical-align:top}
.cellw2 {width:30%;display:table-cell;vertical-align:top}
...
.cellBtn1 {width:100px;text-align:right;display:block;padding-left:5px;letter-spacing:-1px}
...
.inputInfo {position:relative;display:block;}
.inputInfo .inner_g {height:36px;padding:8px 10px 0px 10px;border:2px solid #e0e0e0;}
.inputInfo_id {position:relative;display:block;background:url(../images/common/bg_form.png) no-repeat left -196px}
...
.inputDisable {background:#e0e0e0 no-repeat left 0}
.inputDisable span {display:block;height:44px;font:bold 15px 'dotum';line-height:44px;border:2px solid #e0e0e0;padding-left:10px; background:#e0e0e0}
...
/********************************************************************************************************************************************************
	btn, icon 공통작업
*********************************************************************************************************************************************************/
.btnBasic_green {margin:10px 0 0;background:#006600;text-align:center}
...
.btnBasic_gray a {display:block;height:48px;color:#4c4c4c;font:normal 12px 'dotum';line-height:48px;background:#cacaca no-repeat right -212px}
...
.inputDisable span {display:block;height:44px;color:#666;font:bold 15px 'dotum';line-height:44px;}
/*라디오버튼 줄바꿈시 추가*/
...
<script type="text/javascript">
	...
	ga('set', 'dimension2', 'https://member.pulmuone.co.kr/customer/custModify_R1.jsp?siteno=0002400000&_ga=2.202659284.473100592.1514941314-192977463.1504158895');	
	ga('send', 'pageview', 'custModify_Frm.jsp');
...
<div id="wrap">
	<div id="container_web">
		...
		<div class="gnb gnbDiv3">
			<ul class="clfix">
				<li class="on" id="mh5"><a href="/customer/custModify.jsp?siteno=0002400000">정보수정</a></li>
	<!--			<li class="center" id="mh6"><a href="/customer/siteUseInfo_R1.jsp?siteno=0002400000"><span class="ico_siteAgree"></span> 사이트이용동의 현황</a></li>  -->
				<li id="mh7"><a href="/customer/custSecede.jsp?siteno=0002400000">잇슬림 회원 탈퇴</a></li>
			</ul>
		</div>
		<!-- //gnb -->		
...
<form name="registerForm" id="registerForm" method="post" action="/customer/custModify_proc.jsp">
	...
	<input type="hidden" name="memEmailYn" id="memEmailYn" />
	<input type="hidden" name="emailMod" id="emailMod" />
			...
			<div class="contentWrap">
				...
				<div class="boxBasic">
					...
					<table class="formBasic">
						...
						<tr>
							<td>
								...
								<div class="inputInfo">
								<div class="inner_g">
										<input name="userName" id="userName" title="이름" class="tf_g ww" type="text" style="ime-mode:deactivated;" maxlength="20" value="<%=memName%>">
								</div>
								</div>
							</td>
						</tr>
						<tr>
							<td>
								<p>휴대폰번호</p>
								<!-- hp input -->
								<span id="hpdisable" class="cellFirst">
									<span class="cellw1">
										<div class="inputInfo">
											<div class="inner_g">
												<input name="htel1" id="htel1" title="전화번호 앞자리" class="tf_g ww" type="text" maxlength="4" numberOnly="true" onKeyUp="htelbtnchk();" onClick="htelbtnchk();" value="<%=memHp1%>">
											</div>
										</div>
      								</span>
									...
									<span class="cellw2">
										<div class="inputInfo">
											<div class="inner_g">
												<input name="htel2" id="htel2" title="전화번호 중간자리" class="tf_g ww" type="text" maxlength="4" numberOnly="true" onKeyUp="htelbtnchk();" onClick="htelbtnchk();" value="<%=memHp2%>">
											</div>
										</div>
      								</span>
									<span class="cellDesh">-</span>
									<span class="cellw2">
										<div class="inputInfo">
											<div class="inner_g">
												<input name="htel3" id="htel3" title="전화번호 뒷자리" class="tf_g ww" type="text" maxlength="4" numberOnly="true" onKeyUp="htelbtnchk();" onClick="htelbtnchk();" value="<%=memHp3%>">
											</div>
										</div>
      								</span>      								
									<input type="hidden" name="hidden_send" >
								</span>
								<!-- //input -->
							</td>
						</tr>
						...
						<tr>
							<td>
								<p>이메일</p>
								<!-- email input -->
								<span  id="emaildisable" class="cellFirst">
									<span class="cellw3">
										<div class="inputInfo">
											<div class="inner_g">
												<input name="email1" id="email1" title="이메일 아이디" class="tf_g ww" type="text" value="<%=memEmail1%>">
											</div>
										</div>
      								</span>
									<span class="cellDesh">@</span>	
									<span class="cellw3">
										<div class="inputInfo">
											<div class="inner_g">
												<input name="email2" id="email2" title="이메일 도메인" class="tf_g ww" type="text" value="<%=memEmail2%>">
											</div>
										</div>
										<!-- //select -->
									</span>
					...
					<div id="lastPostsLoader" style="display:none;position:relative;bottom:140px;width:0px;height:0px;" class="loading"><img src="/images/common/loading_txt.gif" alt="loading" /></div>
				...
				<div class="inputStyle_ce"><span class="btn_check plr5"><input type="checkbox" class="ipt_check" name="smsYN" id="smsYN" <% if (smsYn.equals("Y")) { out.print("checked=\"checked\" value=\"Y\""); } else { out.print("value=\"N\""); } %>> <span><label for="smsYN">SMS 수신 동의</label></span></span><span class="btn_check plr5"><input type="checkbox" class="ipt_check" name="emailYN" id="emailYN" <% if (emailYn.equals("Y")) { out.print("checked=\"checked\" value=\"Y\""); } else { out.print(" value=\"N\""); } %>> <span><label for="emailYN">E-mail 수신 동의</label></span></span>
				</div>
				</div>

				<!-- button -->
				<div class="btnBasic_green">
					<a href="javascript:goCustModify();">정보수정 완료</a>	
				</div>	
				<!-- //button -->
			</div>
</form>
