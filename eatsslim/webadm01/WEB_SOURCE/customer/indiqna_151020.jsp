...
function readData(obj,dvLevel){
	...
	oOption.value = "";
	if(dvLevel=="1"){
		ddlRegion=document.getElementById("ct2");
		oOption.text = "==2차선택==";
		for (i = document.getElementById("ct3").length; i >= 0; i--){document.getElementById("ct3").remove(i);}
		initOption(document.getElementById("ct3"));

	}else if(dvLevel==2){
		ddlRegion=document.getElementById("ct3");
		oOption.text = "==3차선택==";
		
		if (document.getElementById("ct1").options[document.getElementById("ct1").selectedIndex].value) {
			pcode=document.getElementById("ct1").options[document.getElementById("ct1").selectedIndex].value;
		}
	}
	...
	if(dvLevel != "4"){ 
		...
		xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded : text/xml;charset=EUC-KR");
		xmlhttp.send(null);
...
%>
						...
						<table class="inputfield" width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td>
										<select name="ct1" id="ct1" onchange="readData(this,1)">
											<%=cateOpt1%>
										</select>
										<select name="ct2" id="ct2" onchange="readData(this,2)">
											<option value=''>==2차선택==</option>
										</select>
										<select name="ct3" id="ct3">
