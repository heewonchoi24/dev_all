...
<div id="wrap">
	...
	<div class="container">
		...
		<form name="frm_order" id="frm_order" method="post">
			...
			<input type="hidden" name="sale_price" id="sale_price" value="5" />
						...
						<ul class="badgecount">
							...
							<li id="addOption">
								...
								<div class="floatright" style="width:140px;" id="optionSelect">
									<select name="add_goods" id="add_goods" style="width:130px;">
										<option value="0" selected="selected">없음</option>
										<option value="2500">벨런스쉐이크 1포</option>
									</select>
									<font style="font-weight:normal; color:#777;">4/24~30일까지 5%할인</font>
								</div>								
								<div class="clear"></div>
							</li>
						</ul>
...
function addCart(t) {
	$("#cart_type").val(t);
	$.post("dietMeal_ajax1.jsp", $("#frm_order").serialize(),
	function(data) {
		$(data).find("result").each(function() {
...
function getGroup(num) {
	...
	function(data) {
		$(data).find("result").each(function() {
			if ($(this).text() == "success") {				
				var addOptions		= '<select name="add_goods" id="add_goods" style="width:130px;" onchange="optionCng();">';
				addOptions		+= '<option value="0" selected="selected">없음</option>';
				addOptions		+= '<option value="2500">벨런스쉐이크 1포</option>';
				addOptions		+= "</select>";
				addOptions		+= '<font style="font-weight:normal; color:#777;">4/24~30일까지 5%할인</font>';
				if (num > 0) {
