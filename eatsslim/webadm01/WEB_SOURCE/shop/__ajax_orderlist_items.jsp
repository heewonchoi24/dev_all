...
query        += " WHERE  ('"+today+"' BETWEEN STDATE AND LTDATE) AND (GROUP_CODE = EGG.GROUP_CODE  OR GROUP_CODE IS NULL) ";
query        += " ORDER BY ES.ID DESC ";
query        += " LIMIT 0, 1) AS BTYPE, GROUP_CODE, GROUP_NAME, OFFER_NOTICE, GROUP_PRICE, GROUP_PRICE1,KAL_INFO, GROUP_INFO1, CART_IMG, GROUP_IMGM, GUBUN2, ID, LIST_VIEW_YN, TAG, DAY_EAT, GUBUN1, DEVL_GOODS_TYPE, DEVL_WEEK3, DEVL_WEEK5";                
...
if(menuF.equals("1") && menuS.equals("1")){
    ...
    if(!eatCoun.equals("")){
        queryCount++;
        query        += " AND SUBSTR(gubun2,2,3) = "+eatCoun;
        query        += " AND ID IN ('43','50','72','46','44','48','107','126')  ";
...
while(rs1.next()){
    ...
    if(groupCode.equals("0301590")){    // 미니밀 경우
        totalPrice = groupPrice1;
    }
    ...
    while (rs.next()) {
        ...
        if (couponSaleType.equals("W")) {// 원 할인
        ...
        if(couponMaxPrice > 0){    // 쿠폰이 있다면
            ...
            //금액계산
            //CouponeSalePrice = paramVala;
			totalPrice = groupPrice1 - paramVala;    
           
            //totalPrice = groupPrice1 - couponMaxPrice; // %세일 계산
        }else {
            totalPrice = groupPrice1;
        }
    ////////////////////////////////////////////////
