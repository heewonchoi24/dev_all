							...
							<table class="tfoot">
								<tfoot>
									<tr>
										...
										<% } else if (couponMaxPrice > 0){ %><span class="event_area"><%=saleTitle%>쿠폰세일</span><% } %>
										</td>
									</tr>
									<tr>
										<td colspan="2">
    ...
    function CalAmount(){
    	if(GroupCode == "0300993"){ //-- 미니밀일경우 한식,양식세트로 적용
	    	...
	    	if(IsDevlWeek && Gubun1 != "02"){ //-- 요일체크이면서 프로그램다이어트가 아닌상품
		   		SaleTotalPrice	= SalePrice * parseInt($("#devlWeek").val()) * (parseInt($("#devlDay").val()) + parseInt($("#devlWeekEnd").val())) * parseInt($("#buyQty").val());
		...
		$("#sumPrice").html(totalComma(SaleTotalPrice - CouponeSalePrice)+"원");
		if(SalePrice != GoodsPrice) {
