...
try {
    ...
    %>
		<script>alert("상품 정보 불러오기를 실패했습니다.");</script>
    <%
    out.println(e+"=>"+query);
    if(true)return;
}
...
while (rs1.next()) {
    ...
    if (couponSaleType.equals("W")) {    // 상품판매	가격의 ~원 할인
    ...
    if("02".equals(gubun1)) {    // 구분1(01: 식사다이어트, 02: 프로그램다이어트, 03: 타입별다이어트)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="inp_thermos">
                                            <input type="checkbox" id="buy_weekend" name="buy_weekend" checked="checked" /> <label for="buy_bag">토 포함</label>
                                            <p>주말식단 선택 시 토 제품(2팩)이 추가되어 금요일에 함께 배송됩니다.</p>
                                        </div>
                                    </td>
                                </tr>
<% } %>
...
%>
                ...
                <div class="detail_inner">
                    <dl>
                        <dt>식단구성 <p>※ 아래 제품 이미지를 클릭하시면, 상세 영양정보를 확인하실 수 있으며, 이해를 돕기위한 사진으로 실 제품과 차이가 있을 수 있습니다.</p></dt>
...
//if(GroupCode == "0301369" || GroupCode == "0301590" || GroupCode == "0301079"){ // 3시 이후되면 1일을 더한다.
    if(Hour >= 15){
        MinDate += 1;
    }
//}
...
function forCoupon(){
    ...
    var orderWeek  = $("#selectDevlWeek").val(); // 선택한 배송 기간

    var paramVala = 0;
    var paramValb = 0;	// 할인가격
    var valueR = 0;
    ...
    for(var r=0; r<orderWeekSize; r++){
        var cp_value = $("#cp_" + r).attr('value');// 쿠폰 주문 기간
        var UseGoods = $("#cp_" + r).attr('usegoods');// 쿠폰 종류(전체, 특정, 일배, 택배)

		var uselimitcnt = $("#cp_" + r).attr('uselimitcnt');// 쿠폰 사용가능 수량 조건
		var uselimitprice = $("#cp_" + r).attr('uselimitprice');// 쿠폰 사용가능 가격 조건
...
//            $("#sumPrice").html(totalComma(GoodsTotalPrice-CouponeSalePrice)+"원");
//        }
		if(CouponeSalePrice > 0){
            $("#orgPrice").html(totalComma(GoodsTotalPrice)+"원");
             $("#sumPrice").html(totalComma(GoodsTotalPrice-CouponeSalePrice)+"원");
        }else{
             $("#sumPrice").html(totalComma(GoodsTotalPrice)+"원");
        }
    }

    var eslMemberId = '<%=eslMemberId%>';
    $("#btnProductViewForm").click(function(){	// 장바구니 담기
           if(eslMemberId != ""){
