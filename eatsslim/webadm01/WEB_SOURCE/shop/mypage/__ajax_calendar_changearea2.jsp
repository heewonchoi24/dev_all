...
rs.close();
%>
<section id="changeArea2" class="modal">
    <div class="inner">
    	<button class="modal_close"></button>
    	<div class="modal_header">
			<h1>배송지 변경 <span>(2/2)</span></h1>
    	</div>
        <div class="modal_content">
        	<div class="inner">
	            ...
	            <div class="bx_edit">
                    <div class="text_area">
                        <strong><%=startYear%>년 <%=startMonth%>월 <%=startDay%>일</strong> 부터 <strong><%=endYear%>년 <%=endMonth%>월 <%=endDay%>일</strong> 까지의 배송지를<br>
                        <span>(<%=rcv_zipcode%>) <%=rcv_addr1%> <%=rcv_addr2%></span> 으로 변경처리 완료하였습니다.<br>감사합니다.
                    </div>
	            </div>
	            <div class="btns center modal_btns">
	            	<button type="button" class="btn white radius20 large" onclick="isSubmit = true;modalFn.hide($('#changeArea2'), $('#changeArea1'));">뒤로가기</button>
	                <button type="button" class="btn black radius20 large" onclick="location.reload();">완료</button>
	                <!-- <button type="button" class="btn black radius20 large" onclick="areaComplete();">완료</button> -->
	            </div>
        	</div>
        </div>
    </div>
