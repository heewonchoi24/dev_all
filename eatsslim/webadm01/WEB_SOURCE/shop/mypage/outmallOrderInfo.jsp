...
String outOrderNum		= ut.inject(request.getParameter("oon"));
String orderName		= ut.inject(request.getParameter("on"));
if(!"".equals(orderName) ) orderName = new String(orderName.getBytes("8859_1"),"euc-kr");
...
String param			= "";
param		= "&page="+ iPage +"&pgsize="+ pgsize +"&ono="+ orderNum +"&oon="+ outOrderNum +"&on="+ orderName;

//if (orderDate.equals("") || orderDate.length() < 1) {
	query		= "SELECT DATE_FORMAT(ORDER_DATE, '%Y.%m.%d') ORDER_DATE, DATE_FORMAT(ORDER_DATE, '%Y%m%d') ORDER_DATE_FROM, DATE_FORMAT(DATE_ADD(ORDER_DATE, INTERVAL 1 MONTH), '%Y%m%d') ORDER_DATE_TO FROM ESL_ORDER WHERE ORDER_NUM = '"+ orderNum +"' AND OUT_ORDER_NUM = '"+ outOrderNum +"'";
...
query		+= " AND O.OUT_ORDER_NUM = '"+ outOrderNum +"'";
							...
							while (rs.next()) {
								...
								if (gubun1.equals("01") || gubun1.equals("02") || rs.getString("DEVL_TYPE").equals("0001")) {
									...
									*/
									...
									dayPrice += goodsPrice;
									devlOpt = "<a href=\"outmallCalendar.jsp?outOrderNum="+outOrderNum+"&jsOrderNum="+orderNum+"&jsGoodsId="+goodsId+"&jsGroupCode="+groupCode+"\">주문조회변경</a>";
			...
			<div class="row">
				<div class="one last col center">
					...
					<div class="button large dark" style="margin:0 10px;">
						<a href="outmallOrder.jsp?<%=param%>">목록으로</a>
					</div>
				</div>
			</div>
		</div>
