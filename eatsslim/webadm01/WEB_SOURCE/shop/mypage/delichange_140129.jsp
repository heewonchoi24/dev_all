...
<div id="wrap">
	...
	<div class="container">
		<div class="maintitle">
			<h1>주문배송 조회</h1>
			<div class="pageDepth"> HOME &gt; 마이잇슬림 &gt; <strong>주문/배송</strong></div>
			<div class="clear"></div>
		</div>
			...
			<div class="one last col">
				<div class="orderSearch">
					<label for="select">주문</label>
						<%
						query		= "SELECT G.GROUP_NAME, G.GROUP_CODE, OG.ORDER_NUM, DATE_FORMAT(O.PAY_DATE, '%Y-%m-%d') PAY_DATE";
						...
						query		+= " ORDER BY OG.ORDER_NUM ASC";
						try {
						...
						<select name="order_num" id="order_num" onChange="cngGoods();">
							...
							%>
							<option value="<%=rsOrderNum%>|<%=rsGroupCode%>|<%=rs.getString("PAY_DATE")%>"<%if (orderNum.equals(rsOrderNum) && groupCode.equals(rsGroupCode)) out.println(" selected=\"selected\"");%>><%=rs.getString("GROUP_NAME")%></option>
							<%
								//}
								i++;
							}
							...
							<tbody>
								<tr>
									...
									<td class="fc-header-center">
										<span class="fc-button fc-button-prev fc-state-default fc-corner-left fc-corner-right" unselectable="on" style="-moz-user-select: none;" onClick="location.href='delichange.jsp?year=<%=preYear %>&month=<%=preMonth%>&ono=<%=orderNum%>&gno=<%=groupCode%>&pdate=<%=payDate%>'">
							...
							<div class="fc-view fc-view-month fc-grid" style="position: relative; -moz-user-select: none;" unselectable="on">
								...
								<tbody>
									...
									for (i = startDay; i <= endDay; i++) {
										...
										캘린더 오류로 공휴일 설정 임시 막음 2013-10-15
										query		= "SELECT HOLIDAY_NAME FROM ESL_SYSTEM_HOLIDAY WHERE HOLIDAY = '"+ nowDate +"'";
										try {
											rs	= stmt.executeQuery(query);
										} catch(Exception e) {
											out.println(e+"=>"+query);
										...
										if (rs.next()) {
											if (addClass.equals(" deliday")) {
												query1		= "SELECT DEVL_YN FROM ESL_MEMBER_HOLIDAY";
												...
												if (rs1.next()) {
													if (rs1.getString("DEVL_YN").equals("Y")) {
														groupName	= "<div class=\"button light small\"><a href=\"javascript:;\">배송거부해제</a></div>";
													} else {
														groupName	= "<div class=\"button light small\"><a href=\"javascript:;\">배송거부설정</a></div>";
													}
												}
											...
											addClass	= " holiday";
											memo		= "<span class=\"memo\">"+ rs.getString("HOLIDAY_NAME") +"</span>";
										}
									...
									*/
									%>
										<td class="fc-day fc-sun fc-widget-content fc-past fc-first<%=addClass%><%if (newLine == 6) out.println(" fc-last");%>" data-date="<%=nowDate%>">
											...
											<%} else {%>
											<%if (groupCode.length()< 7) {%>
											<!--a class="lightbox" href="/shop/popup/foodplan.jsp?lightbox[width]=600&lightbox[height]=350&gid=<%=groupId%>&week=<%=linkWeek%>"-->
												<div>
													<div class="fc-day-number"><%=i%><%=memo%></div>
											...
											<!--/a-->
											<%} else {%>
											<a class="lightbox" href="/shop/popup/foodplan.jsp?lightbox[width]=600&lightbox[height]=350&gid=<%=groupId%>&sdate=<%=nowDate%>">
												<div>
													<div class="fc-day-number"><%=i%><%=memo%></div>
													<div class="fc-day-content"><%=groupName%></div>
												</div>
		...
		<div class="row">
			<div class="one last  col">
				...
				<div class="line-box line-black">
					<p>변경 가능한 배송일과 변경될 배송일 날짜를 입력하신 후 배송일 변경 버튼을 누르셔야 적용이 됩니다.</p>
					<p>배송일 변경은 배송 시작전 D-6일 이후의 일자만 변경이 가능합니다.</p>
					<p>공휴일에 배송을 받지 않으실 경우 배송거부 버튼을 눌러주세요.</p>
				</div>
			</div>
		</div>
		<!-- End row --> 
...
$(document).ready(function() {
	$("#devl_date1, #devl_date2").datepick({ 
...
function cngGoods() {
	...
	location.href = "delichange.jsp?ono="+ orderArr[0] +"&gno="+ orderArr[1] +"&pdate="+ orderArr[2];
}
...
function nextMonth() {
	location.href='delichange.jsp?year=<%=nextYear %>&month=<%=nextMonth%>&ono=<%=orderNum%>&gno=<%=groupCode%>&pdate=<%=payDate%>&week_day=' + $("input[name=week_day]:last").val() + "&sch_week=<%=week%>";
}
</script>
</body>
</html>