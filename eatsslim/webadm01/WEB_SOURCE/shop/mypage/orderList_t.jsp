...
<div id="wrap">
	...
	<div class="container">
		<div class="maintitle">
			<div class="pageDepth">
				<span>HOME</span><span>마이페이지</span><strong>주문내역조회</strong>
			</div>
			<div class="clear"></div>
		</div>
					...
					<div class="one last col">
						<div class="graytitbox orderSearch">
							...
							<div class="floatright">
								<h5>상태</h5>
								<select name="state_type" id="state_type" onChange="document.frm_search.submit();">
					...
					<table class="orderList" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						if (intTotalCnt > 0) {
							while (rs.next()) {
								...
								query1		+= "		DEVL_DAY, DEVL_WEEK, DEVL_PRICE, DATE_FORMAT(DEVL_DATE, '%Y%m%d') DEVL_DATE, OG.ID, G.GROUP_CODE";
								...
								int price			= 0;
								devlType1Count		= 0; //-- 일일배송상품 카운팅
								devlType10301369Count	= 0; //-- 헬씨퀴진 카운팅
								while (rs1.next()) {
									gid = rs1.getString("ID");
									gno = rs1.getString("GROUP_CODE");

									devlType		= rs1.getString("DEVL_TYPE");
									devlDiv			+= "<div class=\"shipping font-blue list\">"+ ut.getDevlType(devlType) +"</div>";
									goodsDiv		+= "<div class=\"list\">"+ut.getGubun1Name(rs1.getString("GUBUN1")) +"<span class=\"option\">("+ ut.getGubun2Name(rs1.getString("GUBUN2")) +": "+ rs1.getString("GROUP_NAME") + ") x "+rs1.getInt("ORDER_CNT")+" </span></div>";
									...
									priceDiv		+= "<div class=\"list\">"+ nf.format(price) +"원</div>";
									...
									if (devlType.equals("0001")) {
										...
										if("0301369".equals(gno) ) devlType10301369Count++; //-- 헬씨퀴진 카운팅
									}
									*/
								}
								rs1.close();
						...
						<tr>
							...
							<%
									...
									if (devlType1Count > 0 && (Integer.parseInt(orderState) == 1 || Integer.parseInt(orderState) == 911)) {
							%>
								<div class="button light small">
									<a href="/shop/mypage/calendar.jsp?ono=<%=orderNum%>&gno=<%=gno%>&pdate=<%=orderDate.replace(".", "-")%>&gid=<%=gid%>">배송일자변경</a>
								</div>
							<%
									}
							%>
