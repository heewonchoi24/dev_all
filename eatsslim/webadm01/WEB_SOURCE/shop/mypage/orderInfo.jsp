...
iPage = iPage.replaceAll("<","&lt;");
iPage = iPage.replaceAll(">","&gt;");
String pgsize		= ut.inject(request.getParameter("pgsize"));
pgsize = pgsize.replaceAll("<","&lt;");
pgsize = pgsize.replaceAll(">","&gt;");
String param			= "";
...
query		= "SELECT OG.GROUP_ID, OG.ORDER_CNT, OG.DEVL_TYPE, OG.DEVL_DAY, OG.DEVL_WEEK, DATE_FORMAT(OG.DEVL_DATE, '%Y.%m.%d') WDATE, OG.PRICE, OG.BUY_BAG_YN, G.GUBUN1, G.GROUP_NAME, G.CART_IMG, O.COUPON_PRICE, OG.ID, G.GROUP_CODE, DEVL_PRICE, EXP_PROMOTION";
...
<%} else {	// 150 프로틴밀	%>
							...
							query		= " SELECT COUNT(B.SET_NAME) CNT_SET_NAME FROM ESL_CART_DELIVERY A, ESL_GOODS_SET B where B.SET_CODE = A.P_SET_CODE AND ORDER_NUM = '"+ orderNum +"' ";
							...
							if(rs1.next()){
								cntpSetName = rs1.getInt("CNT_SET_NAME");
							}
							rs1.close();

							//System.out.println("cntpSetName : " + cntpSetName );

							if(cntpSetName > 0){
								query2		= " SELECT B.SET_NAME, A.BUY_CNT FROM ESL_CART_DELIVERY A, ESL_GOODS_SET B where B.SET_CODE = A.P_SET_CODE AND ORDER_NUM = '"+ orderNum +"' ";
								try {
								...
								while(rs2.next()){
									pSetName = rs2.getString("SET_NAME");
									pBuyCnt = rs2.getInt("BUY_CNT");
								...
								rs2.close();
							}else{	// 단품 수량 기능 개발 전 기존 150 프로틴밀
							%>
								<div><%=orderCnt%></div>
							<%
							}
...
%>
			...
			query		+= "	PG_FINANCENAME, PG_ACCOUNTNUM, ORDER_STATE, AGENCY_NAME, TELNO, HPNO,PG_TID";
				...
				<div class="one last col">
					...
					<table class="line-black paymentinfo" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<th>영수증 출력</th>
							<td>
								<span class="button light small"><a href="#" onclick="window.open('https://pgweb.uplus.co.kr:9091/MpFlowCtrl?eventDiv1=search&eventDiv2=getReceipt&TRANSACTIONID=<%=pgTid%>&SYSTEM=NEW&authResultParam=Y',
								'pop','resizable=no,scrollbars=yes,width=700,height=800');">영수증 출력</a></span>
							</td>
						</tr>
					</table>
...
if(devlType1Count > 0){
	...
	//-- 택배 취소가능일
	cancelDay = 3;
	if( 150000 >= Integer.parseInt(ut.getTimeStamp(10))){ //-- 15시 전까지는 3일 넘으면 2일을 적용한다.
		cancelDay = 2;
	}
}
