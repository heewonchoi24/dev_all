...
%>
<section id="changePattern1" class="modal">
    <div class="inner">
    	<button class="modal_close"></button>
    	<div class="modal_header">
			<h1>배송요일 변경 <span>(1/2)</span></h1>
			<div class="tip_scheduler">배송을 원하는 요일을 선택해주세요.</div>
    	</div>
...
%>
	                         ...
	                         <tr>
	                            <th>수량선택</th>
	                            <td>
	                            	<div class="ipt_group order_cnt">
	                            		<div class="selectbox">
											<label>1개</label>
											<select name="order_cnt" id="order_cnt" onchange="selectbox(this);">
<%
for(int cntNum = 1; cntNum <= productTotalCnt;cntNum++){
%>
                                       			<option value="<%=cntNum%>"><%=cntNum%>개</option>
<%
}
%>
											</select>
	                        ...
	                        <tr>
	                            ...
	                            <td>
									...
									<div class="ipt_group selectDate" onclick="modalFn.show('__ajax_calendar_selectdate.jsp?mindate=<%=startChoiceDate%>&maxdate=<%=maxDate%>&orderNum=<%=orderNum%>&goodsId=<%=goods%>&groupCode=<%=gubunCode%>&caltype=start', {onLoad:calenderSetting1()});">
								    	<input type="text" class="ipt"  id="devl_date_pattern_text" value="<%=startChoiceDateText%>" readonly="">
	                        ...
	                        <tr>
	                            ...
	                            <td>
									...
									<div class="ipt_group selectDate" onclick="modalFn.show('__ajax_calendar_selectdate.jsp?mindate=<%=newStartDate%>&maxdate=<%=maxDate%>&orderNum=<%=orderNum%>&goodsId=<%=goods%>&groupCode=<%=gubunCode%>', {onLoad:calenderSetting2()});">
								    	<input type="text" class="ipt" id="start_date_pattern_text" readonly="">
								    	<span class="ipt_right addon"><button type="button" class="btn"></button></span>
								    </div>
	                            </td>
	                        </tr>
    ...
    <script type="text/javascript">
    	...
    	function calenderSetting1() {
	    	iptTarget = $("#devl_date_pattern");
	    	iptTargetText = $("#devl_date_pattern_text");
	    	parentTarget = $('#changePattern1');
	    }
    	function calenderSetting2() {
	    	iptTarget = $("#start_date_pattern");
	    	iptTargetText = $("#start_date_pattern_text");
	    	parentTarget = $('#changePattern1');
	    }
    	...
    	function updPtDate() {
    		...
    		if(!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test($.trim($('#start_date_pattern').val() ) ) ){
    			alert("적용일자 형식을 확인하세요. ex)0000-00-00");
    			return false;
    		}
    		...
    		function(data) {
    			$(data).find("result").each(function() {
    				if ($(this).text() == "success") {
    					...
    					modalFn.show('__ajax_calendar_changepattern2.jsp?'+param);
