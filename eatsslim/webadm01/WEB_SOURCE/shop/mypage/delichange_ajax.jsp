...
if (mode.equals("editDevl")) {
	...
	//query		= "SELECT DEVL_DAY, DAYOFWEEK('"+ devlDate2 +"') WEEK, (SELECT GUBUN1 FROM ESL_GOODS_GROUP WHERE GROUP_CODE = '"+ groupCode +"') GUBUN1 FROM ESL_ORDER_GOODS WHERE ORDER_NUM = '"+ orderNum +"'";
	//query		+= " AND GROUP_ID = (SELECT ID FROM ESL_GOODS_GROUP WHERE GROUP_CODE = '"+ groupCode +"')";
	query		= "SELECT DEVL_DAY, DAYOFWEEK('"+ devlDate2 +"') WEEK, GUBUN1, GUBUN2 ";
	query		+= " 	FROM ESL_ORDER_GOODS EOG, ESL_GOODS_GROUP EGG";
	query		+= " 	WHERE EOG.GROUP_ID = EGG.ID";
	query		+= " 	AND ORDER_NUM = '"+ orderNum +"'";
	try {
		rs			= stmt.executeQuery(query);
	} catch(Exception e) {
		out.println(e+"=>"+query);
