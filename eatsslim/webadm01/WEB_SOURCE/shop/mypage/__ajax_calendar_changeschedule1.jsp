...
rs.close();
%>
<section id="changeSchedule1" class="modal">
    <div class="inner">
    	<button class="modal_close"></button>
    	<div class="modal_header">
			<h1>배송일정 변경 <span>(1/3)</span></h1>
        ...
        <div class="modal_content">
        	<div class="inner">
	            ...
	            <div class="btns center modal_btns">
	                <button type="button" class="btn sky radius20 large" onclick="updPtDate1();">변경일정 선택</button>
	            </div>
        	</div>
        </div>
    </div>
...
%>
		    $('#pop_scheduler').datepicker({
		        ...
		        inline :    true,
		        language :  "ko",
		        minDate : new Date('<%=startChoiceDate%>'),
		        ...
		        onSelect: function onSelect(fd, date, picker) {
		        	...
		        	if(date.length <= 1){
		        		...
		        		var firstDate = date[0].getDate();
		        		var lastDate = date[1].getDate();
		        		var transferItems = [];
		        		...
		        		dpValue = transferItems.toString();
		           	}
		        }
		    });
    	...
    	function updPtDate1(argument) {
    		if(dpValue == "" || dpValue == null){
    			alert("변경을 원하는 일정을 선택해주세요.");
    			return false;
    		}
    		modalFn.show('__ajax_calendar_changeschedule2.jsp?orderNum=<%=orderNum%>&goodsId=<%=subNum%>&groupCode=<%=gubunCode%>&devlDates=<%=devlDates%>&devlDay=<%=devlDay%>&goods=<%=goods%>');
    	}
    </script>
</section>