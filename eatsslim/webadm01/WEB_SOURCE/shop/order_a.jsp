...
// 									goodsPrice	= price * buyQty;
						...
						<tr>
							...
							<td>
							<% if (couponCnt > 0) { %>
								<select name="coupon_code_<%=groupCode%>" id="coupon_code_<%=groupCode%>" onchange="setPrdtCoupon('<%=groupCode%>')" class="coupon_val">
...
%>
							  <input name="pay_type" type="radio" onclick="viewDiv(this.value);" value="10" required label="결제수단선택" checked="" />신용카드
...
<input type="hidden" name="goods_price" value="<%=cartTotalPrice%>" />
...
/*
	//i = 0;
		$("option").each(function() {
			...
			if (maxCp < couponPrice) {
				maxCp = couponPrice;
				maxVal = $(this).val();
			}
			//i++;
		});
		//$(this).selectedIndex = idxSel;
*/		

		var mySelect = document.getElementById($(this).attr("id"));
		for (i=0; i<mySelect.options.length; i++)
		{
		//	if (mySelect.options[i].text == TYPE_WORD)
		//	{
	//			mySelect.selectedIndex = i;
		//		break;
		//	}
			
			valArr			= mySelect.options[i].value.split("|");
			couponPrice		= parseInt(valArr[1]);
			if (maxCp < couponPrice) {
				maxCp = couponPrice;
				maxVal = mySelect.options[i].value;
			}
		}

/*		
		var cnt = $('select option:selected').val(); 
		if (cnt == 0) {
			//no sel
		} else {
...
*/
		...
		$(this).val(maxVal).attr("selected", "selected");
/*		
		valArr			= $(this).val().split("|");
		couponPrice		= parseInt(valArr[1]);
