...
cpWhere				+= "		(C.USE_LIMIT_CNT <= 0 AND C.USE_LIMIT_PRICE <= "+ payPrice +"),";
cpWhere				+= "		(";
cpWhere				+= "			IF (";
...
%>
							...
							<table class="tfoot">
								<tfoot>
									<tr>
										<th>총 주문금액</th>
										<td>
										<% if (couponMaxPrice > 0) { %><span class="event_area"><%=couponName%></span>
										<% } else if(groupPrice != salePrice){ %><span class="event_area"><%=saleTitle%></span> <% } %>
										</td>
									</tr>
									<tr>
										<td colspan="2">
					...
					<%
						if ( "0331".equals(groupCode) || "0300978".equals(groupCode) ) {
					%>
...
%>
	...
	<div class="detail_inner">
					<dl>
						<dt>이달의 식단 <p>※ 해당 날짜의 제품명을 클릭하시면, 상세 영양정보를 확인하실 수 있습니다.</p></dt>
						<dd class="goods_detail2">
...
var CouponeSalePrice = <%=couponMaxPrice%>;
    ...
    function CalAmount(){
    	if(GroupCode == "0300993"){ //-- 미니밀일경우 한식,양식세트로 적용
	    	...
	    	if(IsDevlWeek && Gubun1 != "02"){ //-- 요일체크이면서 프로그램다이어트가 아닌상품
				...
				if ( CouponSaleType == "P" ) {
					SaleTotalPrice	= (SalePrice - CouponeSalePrice) * parseInt($("#buyQty").val());
				} else {
					SaleTotalPrice	= SalePrice * parseInt($("#buyQty").val()) - CouponeSalePrice;
				}
