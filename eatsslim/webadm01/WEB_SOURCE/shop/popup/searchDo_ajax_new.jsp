...
*/
if (mode.equals("post")) {
	if ((schDoroNm == null || schDoroNm.equals("")) && (schGmNm == null || schGmNm.equals("")) ) {
		code		= "error";
		...
		try {
			...
			query		+= "        , GB_BON_NO     ";
			query		+= "        , GB_BU_NO";
			query		+= "        , NVL(SIDO_NM, '') || ' ' || DECODE(SIGUNGU_NM, null , '', SIGUNGU_NM  || ' ') || DECODE(DECODE(BJD_NM,NULL,HJD_NM, BJD_NM),NULL,OMD_NM, DECODE(BJD_NM,null,HJD_NM,BJD_NM))";
			...
			query		+= "        , DELIVERY_YN";
			query		+= "        , JISA_CD";
			query		+= "        , SATURDAY_YN, DECODE(PARTNERNAME,NULL,' ', PARTNERNAME) AS PARTNERNAME, DECODE(TELNO,NULL,' ', TELNO) AS TELNO, DECODE(HPNO,NULL,' ', HPNO) AS HPNO";
			query		+= "    FROM CM_ZIP_NEW_M CZNM, PHIBABY.V_PARTNER@BM_TO_PHIDM PRTNTBL";
			query		+= "    WHERE 1=1 AND CZNM.JISA_CD = PRTNTBL.PARTNERID(+) ";
			...
			*/
			...
			if (schGmBonNo != null && !schGmBonNo.equals("")) {
				pstmt_bm.setString(qryIdx, schGmBonNo);
				qryIdx++;
			}
			if (schGmBuNo != null && !schGmBuNo.equals("")) {
				pstmt_bm.setString(qryIdx, schGmBuNo);
				qryIdx++;
			}
			...
			while (rs_bm.next()) {
				...
				jisaCd		= rs_bm.getString("JISA_CD");
				gmNo		= rs_bm.getString("GM_NO");
				totalCnt	= rs_bm.getInt("TOTCNT");
				prtnName		= rs_bm.getString("PARTNERNAME");
				prtnTel		= rs_bm.getString("TELNO");
				prtnPhnno		= rs_bm.getString("HPNO");				
				...
				data		+= "<![CDATA["+ devltype1 +"]]>|<![CDATA["+ devltype2 +"]]>|<![CDATA["+ gmNo +"]]>|<![CDATA["+ prtnName +"]]>|<![CDATA["+ prtnTel +"]]>|<![CDATA["+ prtnPhnno +"]]></address>";
