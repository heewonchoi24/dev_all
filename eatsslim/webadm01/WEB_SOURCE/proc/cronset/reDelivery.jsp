...
while (rs_phi2.next()) {
	...
	query1		+= " ) VALUES (";
	query1		+= "	5, TO_CHAR(SYSDATE+"+i+"/(24*60*60), 'yyyymmddHH24MISS'), SYSDATE, SYSDATE, '010-5044-0275', '02-2186-8508', '베이비밀 재배송일 알람', '[풀무원베이비밀]보류해두셨던 \""+ prdNm +"\" "+ lastDate.substring(4, 6)+"-"+lastDate.substring(6, 8) +"부터 재배송 예정! 1. 현관 문고리에 보냉가방을 걸어주세요 2. 재연기를 원한다면 아래 주소 클릭!! 식단확인/일정변경 바로가기- http://me2.do/F9IndPMf";
	try {
...
/*
	...
	query1		+= "	MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE, SUBJECT, MSG_BODY";
	query1		+= " ) VALUES (";
	query1		+= "	5, TO_CHAR(SYSDATE+"+i+"/(24*60*60), 'yyyymmddHH24MISS'), SYSDATE, SYSDATE, '"+ rcvHp +"', '02-2186-8508', '베이비밀 재배송일 알람', '[풀무원베이비밀]보류해두셨던 \""+ prdNm +"\" "+ lastDate.substring(4, 6)+"-"+lastDate.substring(6, 8) +"부터 재배송 예정! 1. 현관 문고리에 보냉가방을 걸어주세요 2. 재연기를 원한다면 아래 주소 클릭!! 식단확인/일정변경 바로가기- http://me2.do/F9IndPMf";
	try {
		stmt1.executeUpdate(query1);
	} catch(Exception e) {
		out.println(e+"=>"+query1);
