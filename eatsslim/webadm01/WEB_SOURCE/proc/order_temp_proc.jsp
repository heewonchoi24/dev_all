...
String rcvAddrBcode	= ut.inject(request.getParameter("rcv_addr_bcode"));
...
if(rs.next()) {
	if (rs.getInt(1) > 0) {
		...
		out.println("<script>top.location.href='"+referer+"'</script>");if(true)return;
...
//주문자명이 "잇슬림"일 때와 Null일 때 수취인 명으로 변경하기
if(orderName.equals("잇슬림") || "".equals(orderName)) 
	orderName = (!"".equals(rcvName)) ? rcvName : tagName;
...
query		+= "	,RCV_BUILDINGNO		= '"+ rcvAddrBcode +"'";
query		+= "	,RCV_HP			= '"+ rcvHp +"'";
...
if (chkCnt > 0 && success_order == true) {
	...
	query		= "SELECT ID, GROUP_ID, BUY_QTY, DEVL_TYPE, DEVL_DAY, DEVL_WEEK, DEVL_WEEKEND, DEVL_DATE, PRICE, BUY_BAG_YN, SS_TYPE, EXP_PROMOTION FROM ESL_CART";
	query		+= " WHERE MEMBER_ID = '"+ eslMemberId +"' AND CART_TYPE = '"+ mode +"'";
	if (oyn.equals("Y")) {
		query		+= " AND ORDER_YN = 'Y'";
	}
	...
	while (rs.next()) {
		...
		if("Y".equals(rs.getString("BUY_BAG_YN") ) ) price -= defaultBagPrice;
		...
		query1		+= "	,DEVL_WEEKEND	= '"+ rs.getString("DEVL_WEEKEND") +"'";
		query1		+= "	,PRICE			= '"+ (price/rs.getInt("BUY_QTY")) +"'";
		...
		if(ut.inject(rs.getString("EXP_PROMOTION")).equals("01")){// 체험단 프로모션
			query1		+= "	,EXP_PROMOTION		= '"+ ut.inject(rs.getString("EXP_PROMOTION")) +"'";
		}
