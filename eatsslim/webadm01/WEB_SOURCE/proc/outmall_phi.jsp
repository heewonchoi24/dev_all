...
if (chkCnt < 1) {
	...
	if (rcvPartner.equals("") || rcvPartner == null || rcvPartner.equals("0") ) {
		// 일배 위탁점 조회
	...
	while (rs.next()) {
		...
		if (gubun2.equals("31") && devlType.equals("0001")) {
			...
			for (int j = 0; j < 500; j++) {
				...
				} else if (devlDay.equals("2")) {
					if (cal.get(cal.DAY_OF_WEEK) == 1 || cal.get(cal.DAY_OF_WEEK) == 2 || cal.get(cal.DAY_OF_WEEK) == 4 || cal.get(cal.DAY_OF_WEEK) == 6 || cal.get(cal.DAY_OF_WEEK) == 7) { //월수금토일요일은 배송을 하지 않는다
					} else {
						devlDatePhi		= dt.format(cal.getTime());
				...
				} else if (devlDay.equals("1")) {
					if (cal.get(cal.DAY_OF_WEEK) == 1 || cal.get(cal.DAY_OF_WEEK) == 7) { //토일요일은 배송을 하지 않는다
					} else {
						devlDatePhi		= dt.format(cal.getTime());
