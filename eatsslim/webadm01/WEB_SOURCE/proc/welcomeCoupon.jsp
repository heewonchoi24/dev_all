...
// À£ÄÄÄíÆù ¾Ë¶óÇï¾¾/¾Ë¶ó½½¸² »óÇ° 5Ãµ¿ø Àû¿ë
query		= "SELECT ID FROM "+ cpTable +" WHERE COUPON_NAME = 'ÀÕ½½¸² À£ÄÄÄíÆù_¾Ë¶ó 5Ãµ¿ø ÇÒÀÎ±Ç' AND STDATE = '"+ stdate +"' AND LTDATE = '"+ ltdate +"'";
try {
	rs		= stmt.executeQuery(query);
} catch(Exception e) {
	out.println(e+"=>"+query);
...
if (couponId > 0) {
	...
	query		+= "	(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
	pstmt		= conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
	pstmt.setString(1, "ÀÕ½½¸² À£ÄÄÄíÆù_¾Ë¶ó 5Ãµ¿ø ÇÒÀÎ±Ç");
	pstmt.setString(2, "01");
	pstmt.setString(3, stdate);
	pstmt.setString(4, ltdate);
	pstmt.setString(5, "02");
...
query		= "SELECT COUNT(CM.ID) FROM ESL_COUPON C, ESL_COUPON_MEMBER CM";
query		+= " WHERE C.ID = CM.COUPON_ID AND CM.MEMBER_ID = '"+ memberId +"' AND C.COUPON_NAME like '%ÀÕ½½¸² À£ÄÄÄíÆù_¾Ë¶ó%'";
try {
	rs		= stmt.executeQuery(query);
} catch(Exception e) {
	out.println(e+"=>"+query);
...
// À£ÄÄÄíÆù Å¬·»ÁîÇÁ·Î±×·¥ 2¸¸¿ø Àû¿ë
query		= "SELECT ID FROM "+ cpTable +" WHERE COUPON_NAME = 'ÀÕ½½¸² À£ÄÄÄíÆù_Å¬·»ÁîÇÁ·Î±×·¥ 2¸¸¿ø ÇÒÀÎ±Ç' AND STDATE = '"+ stdate +"' AND LTDATE = '"+ ltdate +"'";
try {
	rs		= stmt.executeQuery(query);
} catch(Exception e) {
	out.println(e+"=>"+query);
...
if (couponId > 0) {
	...
	query		+= "	(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
	pstmt		= conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
	pstmt.setString(1, "ÀÕ½½¸² À£ÄÄÄíÆù_Å¬·»ÁîÇÁ·Î±×·¥ 2¸¸¿ø ÇÒÀÎ±Ç");
	pstmt.setString(2, "01");
	pstmt.setString(3, stdate);
	pstmt.setString(4, ltdate);
	pstmt.setString(5, "02");
...
query		= "SELECT COUNT(CM.ID) FROM ESL_COUPON C, ESL_COUPON_MEMBER CM";
query		+= " WHERE C.ID = CM.COUPON_ID AND CM.MEMBER_ID = '"+ memberId +"' AND C.COUPON_NAME like '%ÀÕ½½¸² À£ÄÄÄíÆù_Å¬·»ÁîÇÁ·Î±×·¥ 2¸¸¿ø%'";
try {
	rs		= stmt.executeQuery(query);
} catch(Exception e) {
	out.println(e+"=>"+query);
