		...
		if(code.equals("CD1")){ //취소완료 시
			if(pgTID!=null && pgTID.length()>0){ //pg사 취소
				if(payType.equals("30")){ //가상계좌는 사용자환불정보를 PG가에 넘김
					...
					addAct="location.href='../xpay/Cancel.jsp?LGD_TID="+pgTID+"&LGD_RFBANKCODE="+ut.getBankCode(bankName)+"&LGD_RFACCOUNTNUM="+bankAccount+"&LGD_RFCUSTOMERNAME="+bankUser+"&LGD_RFPHONE="+LGD_RFPHONE+"&cancelFrom="+cancelFrom+"';";
				}else{
					addAct="location.href='../xpay/Cancel.jsp?LGD_TID="+pgTID+"&cancelFrom="+cancelFrom+"';";
				}
			}
		}
