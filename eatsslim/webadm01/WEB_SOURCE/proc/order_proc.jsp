...
if(orderEnv.equals("M")){ //모바일주문일 경우
	returnURL="parent.location.href='/mobile/shop/payment.jsp?ono="+orderNum+"'";
}else{
...
//상품명
query1		= "SELECT GROUP_ID, GROUP_NAME, RCV_HP, DEVL_DATE, DEVL_TYPE, DEVL_FIRST_DAY, EXP_PROMOTION";
...
while (rs1.next()) {
	...
	devlFirstDay	= rs1.getInt("DEVL_FIRST_DAY");
	promotion		= ut.inject(rs1.getString("EXP_PROMOTION"));// 체험단 프로모션
	
	Date date		= cdt2.parse(devlDate);
	...
	if (devlType.equals("0001")) {
		...
		} else if ( closeDateType != 1 && (groupId == 32 || groupId == 40 || groupId == 69 || groupId == 92 || groupId == 110) ) {
...
if (payType.equals("30")) { //가상계좌는 주문접수로
	...
	if(tagHp != null && !"".equals(tagHp)){
		...
		query		+= " ,'[풀무원잇슬림] 주문완료'";
		query		+= " ,'[풀무원잇슬림] \""+orderNum+"\" \""+ productName + "\" 주문완료! " + pgCloseDate.substring(4, 6) + "/" + pgCloseDate.substring(6, 8) + " " + pgCloseDate.substring(8, 10) + "시까지" + " 입금기한이 지나면 자동 주문 취소되오니 꼭 기한내에 입금부탁드립니다. ^^'";
		query		+= " ,'001','002','004','"+tagHp+"'";
		query		+= " ,'02-6411-8322','R00',SYSDATE,'eat1',SYSDATE";
		query		+= " ,'kakao_es','N','잇슬림바로가기','http://www.eatsslim.co.kr','','')";
		//System.out.println("::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");
		//System.out.println("3-1 new : " + query);
	...
	if(rcvHp != null && !"".equals(rcvHp) || tagHp != null && !"".equals(tagHp)){
		if(tagHp != null && !"".equals(tagHp)){
			rcvHp = tagHp;
		}
...
//			System.out.println("1");
			...
			query		+= " ) VALUES (";
			query		+= " CUST_DELV_ALL.TSMS_MESSAGE_SEQ.nextval ,'1710002662'";
			query		+= " ,'[풀무원잇슬림] 잇슬림 주문이 완료되었습니다.\r▶ 주문제품 : "+ productName + "\r▶ 배송기간 : " + minDate + "~" +maxDate + "\r\r※ 주문 조회/ 배송 일정/ 배송지 변경은 아래 링크된 고객기쁨센터 또는 잇슬림 홈페이지에서 가능합니다.'";
			query		+= " ,'[풀무원잇슬림] 배송안내'";
			query		+= " ,'[풀무원잇슬림] 잇슬림 주문이 완료되었습니다.\r▶ 주문제품 : "+ productName + "\r▶ 배송기간 : " + minDate + "~" +maxDate + "\r\r※ 주문 조회/ 배송 일정/ 배송지 변경은 아래 링크된 고객기쁨센터 또는 잇슬림 홈페이지에서 가능합니다.\rhttp://www.eatsslim.co.kr'";
			query		+= " ,'001','002','004','"+rcvHp+"'";
			query		+= " ,'02-6411-8322','R00',SYSDATE,'eat10',SYSDATE";
			query		+= " ,'kakao_es','N','잇슬림바로가기','http://www.eatsslim.co.kr','','')";
			//System.out.println("::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");
			...
			if(devlType1Count > 0){
				...
				query		+= " ) VALUES (";
				query		+= " CUST_DELV_ALL.TSMS_MESSAGE_SEQ.nextval ,'1710002662'";
				query		+= " ,'[풀무원잇슬림]\r잇슬림 주문이 완료되었습니다.\r\r▶ 주문제품 : "+ productName + "\r▶ 배송기간 : " + minDate + "~" +maxDate + "\r\r※ 주문조회/ 배송 일정/ 배송지 변경은 잇슬림 홈페이지를 통해 이용가능합니다.\r→ 잇슬림 홈페이지 > 마이페이지 > 주문조회/변경'";
				query		+= " ,'[풀무원잇슬림] 배송안내'";
				query		+= " ,'[풀무원잇슬림]\r잇슬림 주문이 완료되었습니다.\r\r▶ 주문제품 : "+ productName + "\r▶ 배송기간 : " + minDate + "~" +maxDate + "\r\r※ 주문조회/ 배송 일정/ 배송지 변경은 잇슬림 홈페이지를 통해 이용가능합니다.\r→ 잇슬림 홈페이지 > 마이페이지 > 주문조회/변경'";
				query		+= " ,'001','002','004','"+rcvHp+"'";
				query		+= " ,'02-6411-8322','R00',SYSDATE,'eat9',SYSDATE";
				query		+= " ,'kakao_es','N','잇슬림바로가기','http://www.eatsslim.co.kr','','')";
				
			}
			else{
//				System.out.println("택배상품");
				query		= "INSERT INTO CUST_DELV_ALL.TSMS_AGENT_MESSAGE (";
				query		+= " MESSAGE_SEQNO, SERVICE_SEQNO";
				query		+= " ,SEND_MESSAGE";
				query		+= " ,SUBJECT";
				...
				query		+= " ) VALUES (";
				query		+= " CUST_DELV_ALL.TSMS_MESSAGE_SEQ.nextval ,'1710002662'";
				query		+= " ,'[풀무원잇슬림]\r잇슬림 주문이 완료되었습니다.\r\r▶ 주문제품 : "+ productName + "\r▶배송기간 : " + minDate + "~" +maxDate + "\r배송일은 주말, 공휴일, 택배사 사정 등에 따라 일부 지연될 수 있습니다.\r\r\r※ 주문조회/ 배송 일정/ 배송지 변경은 잇슬림 홈페이지(www.eatsslim.co.kr) 에서 가능합니다.'";
				query		+= " ,'[풀무원잇슬림] 배송안내'";
				query		+= " ,'[풀무원잇슬림]\r잇슬림 주문이 완료되었습니다.\r\r▶ 주문제품 : "+ productName + "\r▶배송기간 : " + minDate + "~" +maxDate + "\r배송일은 주말, 공휴일, 택배사 사정 등에 따라 일부 지연될 수 있습니다.\r\r\r※ 주문조회/ 배송 일정/ 배송지 변경은 잇슬림 홈페이지(www.eatsslim.co.kr) 에서 가능합니다.'";
				query		+= " ,'001','002','004','"+rcvHp+"'";
...
if(promotion.equals("01")){// 체험단 프로모션
	query		= "UPDATE ESL_ORDER_GOODS SET PRICE = 0, ORDER_STATE='"+ orderState +"' WHERE ORDER_NUM = '"+ orderNum +"'";
}else{
...
})(window, document, '//rum.beusable.net/script/b170626e173427u675/61dec39e0a');
</script>
</body>
</html>
<!-- end -->