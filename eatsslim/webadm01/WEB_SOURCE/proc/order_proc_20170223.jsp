...
if (payType.equals("30")) { //가상계좌는 주문접수로
	...
	query		+= " ) VALUES (";
	query		+= "	5, now()+0, now(), now(), '"+ rcvHp +"', '02-6411-8322', '[풀무원잇슬림]', '[풀무원잇슬림] \""+orderNum+"\" \""+ productName + "\" 주문완료! " + pgCloseDate.substring(4, 6) + "/" + pgCloseDate.substring(6, 8) + " " + pgCloseDate.substring(8, 10) + "시까지" + " 입금기한이 지나면 자동 주문 취소되오니 꼭 기한내에 입금부탁드립니다. ^^')";
	try {
	...
	query		+= "	MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE, SUBJECT, MSG_BODY";
	query		+= " ) VALUES (";
	query		+= "	5, now()+0, now(), now(), '"+ rcvHp +"', '02-6411-8322', '[풀무원잇슬림]', '[풀무원잇슬림] 잇슬림 주문이 완료되었습니다. \""+ productName + "\" \"" + minDate + "~" +maxDate + "\"동안 배달 예정입니다. 식단 안내 : 잇슬림 이달의 식단 스케쥴 URL 안내 ( http://www.eatsslim.co.kr/mobile/intro/schedule.jsp ) 배송일 변경은 받으시는 날 3일전까지 잇슬림 홈페이지 \"마이페이지>주문/배송>배송일자변경\" 또는 1:1문의하기 통하여 가능합니다. 감사합니다.')";
	try {
		stmt.executeUpdate(query);
	} catch(Exception e) {
		out.println(e+"=>"+query);
