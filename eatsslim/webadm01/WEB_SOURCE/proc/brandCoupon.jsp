...
if (orderId > 0) {
	...
	query		= "SELECT ID FROM "+ cpTable +" WHERE COUPON_NAME = '잇슬림 4월_브랜드위크 5% 할인권' AND STDATE = '20160401' AND LTDATE = '20160407'";
	try {
		rs		= stmt.executeQuery(query);
	} catch(Exception e) {
		out.println(e+"=>"+query);
	...
	query		+= " WHERE C.ID = CM.COUPON_ID AND CM.MEMBER_ID = '"+ eslMemberId +"' AND C.COUPON_NAME like '%4월_브랜드위크 5%%'";
	try {
		rs		= stmt.executeQuery(query);
	} catch(Exception e) {
		out.println(e+"=>"+query);
