...
if (request.getParameter("id") != null && request.getParameter("id").length() > 0) {
	...
	//댓글수
	query		= "SELECT COUNT(ID) FROM ESL_EVENT_HOUSEWIFE_REPLY where ID="+noticeId+"";
...
<script src="housewifeView.js"></script>
</head>
<body>
<div id="wrap">
	<div id="header">
	...
	<div class="container">
		<div class="maintitle">
			<h1>워킹맘&amp;가사맘 다이어트 스토리</h1>
			<div class="pageDepth">
				HOME &gt; 이벤트 &gt; <strong>워킹맘&amp;가사맘 다이어트 스토리</strong>
			</div>
			<div class="clear"></div>
		</div>
						...
						<div class="post-read">
							...
							<ul class="meta-wrap">
								...
								<div class="share floatright">
									<ul>
										<li> <a class="facebook" href="http://www.facebook.com/share.php?u=<%=request.getRequestURL()%>?id=<%=noticeId%>" target="_blank"></a> </li>
										<li> <a class="twitter" href="http://twitter.com/share?url=<%=request.getRequestURL()%>?id=<%=noticeId%>&text=eatsslim diet" target="_blank"></a> </li>
										<li> <a class="me2day" href="http://me2day.net/posts/new?new_post[body]=<%=request.getRequestURL()%>?id=<%=noticeId%>" target="_blank"></a> </li>
										<li> <a class="print" href="#" onClick="window.print();return false;"></a> </li>
									</ul>
								</div>
							</ul>
							<div class="clear"></div>
							...
							<div class="col center">
								<div class="button small dark">
									<a href="/event/housewife.jsp?<%=param%>">목록</a>
								</div>
							</div>
							<div class="comment-wrap">
								...
								if(reply_idx > 0){
									query		= "SELECT * ";
									query		+= " FROM ESL_EVENT_HOUSEWIFE_REPLY where ID="+noticeId+" and IDX="+reply_idx+" and M_ID='"+eslMemberId+"'";
								...
								<ul>
									<li class="comment depth-1" >
										<%
										query		= "SELECT * ";
										query		+= " FROM ESL_EVENT_HOUSEWIFE_REPLY where ID="+noticeId+"";
										query		+= " ORDER BY IDX ASC";
										...
										<div class="commentheader">
											<h5><%=m_id%></h5>
											<div class="metastamp">
												<%=reply_date.substring(0,16)%>
											</div>
											<div class="myadmin">
												<% if(m_id.equals(eslMemberId)){ %>
												<a href="/event/housewifeView.jsp?<%=param%>&id=<%=noticeId%>&reply_idx=<%=reply_idx%>">수정</a> ㅣ <a href="javascript:reply_del('<%=reply_idx%>');">삭제</a>
												<% } %>
											</div>
										</div>
		...
		<div class="sidebar four columns">
			<div class="colums-side">
				<ul class="listNavi">
					...
					<li class="active"><a href="/event/housewife.jsp?<%=param%>"><span class="current"></span>목록</a></li>
					<li class="last">
						<%
