...
//================PHI 연동
for (int mm = 0 ; mm<order_num.length ; mm++) {
	int ordSeq			= 0;

	query		= "UPDATE PHIBABY.P_ORDER_MALL_PHI_ITF SET INTERFACE_FLAG_YN = 'Y' WHERE ORD_NO = '"+ order_num[mm] +"'";
	try {
	...
	query		= "SELECT * FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ order_num[mm] +"' AND STATE < 90";
	try {
	...
	while (rs.next()) {
		...
		pstmt_phi.setString(3, order_num[mm]);
