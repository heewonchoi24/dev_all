...
String content				= ut.inject(multi.getParameter("recontent"));
...
System.out.println("coupon_num_m1: " + coupon_num_m1);
System.out.println("coupon_num_m2: " + coupon_num_m2);
System.out.println("coupon_num_m3: " + coupon_num_m3);
...
if (mode != null) {
	if (mode.equals("write")) {
        ...
        if ( noticeId == 257 ) {
            ...
            query        += " WHERE C.ID = CM.COUPON_ID AND CM.MEMBER_ID = '"+ m_id +"' AND C.ID in (3184, 3185, 3186)";
            try {
            ...
            if (couponCnt > 0) {   
                ...
                query        += " VALUES (3184,'"+ couponNum + "001" +"','"+ m_id +"','N',NOW())";
                try {
                ...
                query        = "INSERT INTO ESL_COUPON_MEMBER (COUPON_ID, COUPON_NUM, MEMBER_ID, USE_YN, INST_DATE)";
                query        += " VALUES (3185,'"+ couponNum + "002" + "','"+ m_id +"','N',NOW())";
                try {
                    stmt.executeUpdate(query);
                } catch (Exception e) {
                    out.println(e);
                ...
                query        = "INSERT INTO ESL_COUPON_MEMBER (COUPON_ID, COUPON_NUM, MEMBER_ID, USE_YN, INST_DATE)";
                query        += " VALUES (3186,'"+ couponNum + "003" +"','"+ m_id +"','N',NOW())";
                try {
                    stmt.executeUpdate(query);
                } catch (Exception e) {
                    out.println(e);
            ...
            out.print("<script>alert('쿠폰 발급이 완료되었습니다. 쿠폰은 마이페이지에서 확인하실 수 있습니다.');location.href='view.jsp?"+param+"';</script>");
			...
			if(dw_yn.equals("RDW")){// 댓글 등록 시 쿠폰 발급의 경우
				...
				if(repleCnt < 1){
					...
					if(!coupon_num_m1.equals("")){
						try {
							query        = "INSERT INTO ESL_COUPON_MEMBER (COUPON_ID, COUPON_NUM, MEMBER_ID, USE_YN, INST_DATE)";
							query        += " VALUES ('" + coupon_num_m1 + "', '"+ couponNum + "001" +"', '" + m_id + "', 'N', NOW())";
							stmt.executeUpdate(query);
					...
					if(!coupon_num_m2.equals("")){
						try {
							query        = "INSERT INTO ESL_COUPON_MEMBER (COUPON_ID, COUPON_NUM, MEMBER_ID, USE_YN, INST_DATE)";
							query        += " VALUES ('" + coupon_num_m2 + "', '"+ couponNum + "001" +"', '" + m_id + "', 'N', NOW())";
							stmt.executeUpdate(query);
					...
					if(!coupon_num_m3.equals("")){
						try {
							query        = "INSERT INTO ESL_COUPON_MEMBER (COUPON_ID, COUPON_NUM, MEMBER_ID, USE_YN, INST_DATE)";
							query        += " VALUES ('" + coupon_num_m3 + "', '"+ couponNum + "001" +"', '" + m_id + "', 'N', NOW())";
							stmt.executeUpdate(query);
				   ...
				   out.print("<script>alert('댓글이 등록되었습니다.');location.href='view.jsp?"+param+"';</script>");
				}
			}
			else{// 무조건 쿠폰 발급의 경우
				try {
		...
		out.print("<script>alert('수정되었습니다');location.href='view.jsp?"+param+"';</script>");
		...
		if(dw_yn.equals("RDW")){// 댓글 등록 시 쿠폰 발급의 경우
			...
			if(repleCnt <= 1){// 댓글 경험이 없을 경우(댓글을 모두 삭제 하였을 경우)
				...
				try {
					query        = "DELETE FROM ESL_COUPON_MEMBER WHERE MEMBER_ID = '" + m_id + "' AND COUPON_ID IN ('" + coupon_num_m1 + "', '" + coupon_num_m2 + "', '" + coupon_num_m3 + "')";
					pstmt        = conn.prepareStatement(query);
					System.out.println("query22: "+query );
					pstmt.executeUpdate();
			...
			out.print("<script>alert('삭제되었습니다');location.href='view.jsp?"+param+"';</script>");
		}else{
