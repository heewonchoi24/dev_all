...
query		+= " 		DEVL_GOODS_TYPE, DEVL_FIRST_DAY, DEVL_MODI_DAY, DEVL_WEEK3, DEVL_WEEK5, CATE_CODE, DAY_EAT, GUBUN1, DEVL_GOODS_TYPE, DEVL_WEEK3, DEVL_WEEK5";
query		+= " FROM ESL_GOODS_GROUP EGG";
query		+= " WHERE USE_YN = 'Y' ";
query		+= " AND LIST_VIEW_YN = 'Y' ";
query		+= " AND TAG LIKE '%,06,%' ";
query		+= " ORDER BY TAG_SORT LIMIT 4";
...
while(rs.next() ){
    ...
    devlGoodsType = ut.isnull(rs.getString("DEVL_GOODS_TYPE") );// 택배, 일배 구분
    devlWeek3   = ut.isnull(rs.getString("DEVL_WEEK3") );
    devlWeek5   = ut.isnull(rs.getString("DEVL_WEEK5") );

	if(maingroupCode.equals("0301844")){
		maingroupPrice1 = maingroupPrice;
	}
...
//	int payPrice		= 0;
	...
	query        += "        FROM "+ cpTable;
	query        +=            cpWhere;
	query        += "        AND CM.COUPON_ID IN (SELECT COUPON_ID FROM ESL_COUPON_GOODS WHERE GROUP_CODE = '"+ maingroupCode +"')";
	query        += "        ) X ";
	//query1        += " WHERE DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i') BETWEEN STDATE AND LTDATE";
	...
	while (rs1.next()) {
		...
		if (couponSaleType.equals("W")) {
			...
			couponPrice		= Integer.parseInt(String.valueOf(Math.round((double)maingroupPrice1 * (double)couponeSalePrice / 100)));
		}
	...
	if(couponMaxPrice > 0){		
		...
		if( useLimitCnt <= 1 && useLimitPrice <= maingroupPrice1 ){
        ...
        //금액계산
		maintotalPrice = maingroupPrice1 - paramVala; // %세일 계산
					...
					<li><a href="/mobile/shop/order_view.jsp?cateCode=0&cartId=<%=maincartId%>&groupCode=<%=maingroupCode %>">
						...
						<div class="info">
							...
							<p class="desc">
								<span class="price">
								<%if(maintotalPrice == 0){%>
									<span><strong><%=ut.getComma(maingroupPrice1)%>원</strong></span>
								<%}else{%>
...
%>
	...
	<!-- 2017.08.08 삭제
		...
		<div class="row last">
			<h2 class="title">#eatsslim_<p class="subtt"><a href="https://www.instagram.com/_eatslim_/?ref=badge" target="_blank">잇슬림 인스타그램 바로가기 ></a></p></h2>
...
<script type="text/javascript">
	...
	var list = "";
	$.ajax({
		url : 'https://api.instagram.com/v1/users/self/media/recent/?access_token=2100633077.84cd584.982614d62d4a4acaa22656ff495327f4',
		type:"POST",
		async : false,
		dataType : "jsonp",
