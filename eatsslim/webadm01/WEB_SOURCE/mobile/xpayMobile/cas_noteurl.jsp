    ...
    if (LGD_HASHDATA2.trim().equals(LGD_HASHDATA)) { //해쉬값 검증이 성공이면
        if ( ("0000".equals(LGD_RESPCODE.trim())) ){ //결제가 성공이면
        	if( "R".equals( LGD_CASFLAG.trim() ) ) {
				...
				query="UPDATE ESL_ORDER SET PG_ESCROW_YN = '"+LGD_ESCROWYN +"' WHERE ORDER_NUM='"+LGD_OID+"'";
				...
				if(!LGD_CASCAMOUNT.equals("0")){
					if(LGD_CASCAMOUNT.equals(LGD_CASTAMOUNT)){ //총액과 입금액이 같은면 완납
						//query="UPDATE ESL_ORDER SET PAY_DATE=NOW(),ORDER_STATE='01',PAY_YN='Y',ORDER_LOG=CONCAT(ORDER_LOG,'\nPG사 무통장입금완료(총액:"+LGD_CASTAMOUNT+",입금액:"+LGD_CASCAMOUNT+") ',NOW()) WHERE ORDER_NUM='"+LGD_OID+"'";
						//try{stmt.executeUpdate(query);	}catch(Exception e){out.println(e);if(true)return;}
	
						query="UPDATE ESL_ORDER SET PAY_DATE=NOW(),ORDER_STATE='01',PAY_YN='Y', PG_TID = '"+ LGD_TID +"'";
						if ( "Y".equals( LGD_ESCROWYN.trim() ) ) {
							query+="', PG_ESCROW_YN = '"+LGD_ESCROWYN+"'";
						}
...
<%
           ...
           //if( 결제실패 상점처리결과 성공 ) 
        	  // resultMSG = "OK";     
			resultMSG = "<script>alert('결제 오류가 발생하였습니다. 오류코드="+LGD_RESPCODE.toString()+"');</script>";
        }
    } else { //해쉬값이 검증이 실패이면
        /*
         * hashdata검증 실패 로그를 처리하시기 바랍니다. 
         */      
         resultMSG = "<script>alert('결제결과 상점 DB처리(LGD_CASNOTEURL) 해쉬값 검증이 실패하였습니다.');</script>";
    }
