...
<%
            ...
            //if( 결제성공 상점처리결과 성공 ) 
				...
				/*sql="update gd_order set cdt=now(),cyn='Y',state=1 where ordno='"+LGD_OID+"'";
				try{stmt.executeUpdate(sql);}catch(Exception e){out.println(e+"=>"+sql);if(true)return;}	

				sql="update gd_order_item set istate=1 where ordno='"+LGD_OID+"'";
				try{stmt.executeUpdate(sql);}catch(Exception e){out.println(e+"=>"+sql);if(true)return;}	*/
         ...
         */      
        resultMSG = "<script>alert('결제결과 상점 DB처리(NOTE_URL) 해쉬값 검증이 실패하였습니다.');</script>";     
    }
