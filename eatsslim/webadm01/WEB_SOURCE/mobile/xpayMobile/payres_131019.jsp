...
request.setCharacterEncoding("euc-kr");
	...
	configPath="/home/webadm01/esl_admin/lgdacom";
    /*
     *************************************************
     * 1.최종결제 요청 - BEGIN
     ...
     if ( xpay.TX() ) {
		 ...
		 */
         ...
         if( "0000".equals( xpay.m_szResCode ) ) {
			...
			if("<%=xpay.Response("LGD_PAYTYPE",0)%>"=="SC0010"){ //신용카드
				f.pay_type[0].checked=true;
			}else if("<%=xpay.Response("LGD_PAYTYPE",0)%>"=="SC0030"){ //계좌이체
				f.pay_type[1].checked=true;
			}else if("<%=xpay.Response("LGD_PAYTYPE",0)%>"=="SC0040"){ //가상계좌(무통장입금)
				f.pay_type[2].checked=true;
			}
			if(opener.document.getElementById('payBtn'))opener.document.getElementById('payBtn').style.display="none";
			if(opener.document.getElementById('pay_ing'))opener.document.getElementById('pay_ing').style.display="";

			alert("주문결제완료 페이지가 나타날때까지 잠시 기다려 주십시오.");			
			opener.payMobileComplete("<%=xpay.Response("LGD_OID",0)%>");
