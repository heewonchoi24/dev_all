...
<%
	...
	String devlWeekEnd	    = ut.inject(request.getParameter("devlWeekEnd")); //-- 일일배송 주말여부
	...
	/* 150 프로틴밀 시작 */
	int p_buy_qty		= 0; // 전체 수량
	String[] pBuyQty;		 // 각 수량 담을 배열 선언
	String[] pSetCode;		 // 각 세트 코드 담을 배열 선언
	/* 150 프로틴밀 끝 */
	...
	try{
		...
		/* 150 프로틴밀 시작 */
		p_buy_qty		= Integer.parseInt(request.getParameter("p_buy_qty"));
		pBuyQty			= request.getParameterValues("buy_qty");
		pSetCode		= request.getParameterValues("p_set_code");
		/* 150 프로틴밀 끝 */
	...
	if("".equals(devlWeekEnd) ) devlWeekEnd  = "0"; // 일일배송이 아닐경우는 Null값이다 0으로 설정
	...
	*/
	if (devlType.equals("0001")  && day.equals("") ) {
		code		= "error";
		data		= "날짜를 입력하셔야 합니다..";
	}
	else{
		...
		if (cartId > 0) { //-- 장바구니에 있다면 수정
			query		= "UPDATE ";
			query		+= " ESL_CART ";
			query		+= " SET DEVL_DAY = ? , DEVL_WEEK = ? , PRICE = ? , DEVL_DATE = ? , BUY_QTY = ?, BUY_BAG_YN = ?, DEVL_WEEKEND = ? WHERE ID = ?";
			pstmt		= conn.prepareStatement(query);
			...
			pstmt.setString(7, devlWeekEnd);
			pstmt.setInt(8, cartId);
			...
			query		+= "	(MEMBER_ID, GROUP_ID, BUY_QTY, DEVL_TYPE, DEVL_DAY, DEVL_WEEK, DEVL_DATE, PRICE, INST_DATE, BUY_BAG_YN, CART_TYPE, DEVL_WEEKEND)";
			query		+= " VALUES";
			query		+= "	(?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?)";
			pstmt		= conn.prepareStatement(query);
			...
			pstmt.setString(11, devlWeekEnd);
			pstmt.executeUpdate();
			code		= "success";
		...
		//-- 150 프로틴밀 ESL_CART_DELIVERY에 등록
		if("0301844".equals(groupCode) && cartId > 0){
			query		= "DELETE FROM ESL_CART_DELIVERY WHERE CART_ID = "+ cartId;
			stmt.executeUpdate(query);

			if (cartId > 0) {

				for(int p=0; p<pBuyQty.length ; p++){
					query		= "INSERT INTO ESL_CART_DELIVERY ";
					query		+= "	(CART_ID, GROUP_CODE, P_SET_CODE, BUY_CNT, GROUP_ID)";
					query		+= " VALUES";
					query		+= "	(?, ?, ?, ?, ?)";
					pstmt		= conn.prepareStatement(query);
					pstmt.setInt(1, cartId);
					pstmt.setString(2, groupCode);
					pstmt.setString(3, pSetCode[p]);
					pstmt.setInt(4, Integer.parseInt(pBuyQty[p]));
					pstmt.setInt(5, groupId);// 123
					pstmt.executeUpdate();
				}
			}
		}
