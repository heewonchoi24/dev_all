...
<%
        	...
        	<div class="opt_select_date ty2">
        		...
        		<span onclick="cal_on();">
	            	<input type="text" class="dayText"  id="dayText" name="day_cron" value="<%=cartMap.get("DEVL_DATE")%>" readonly>
	            	<input type="hidden" id="opt_day" name="day" value="<%=cartMap.get("DEVL_DATE")%>">
...
%>
	...
	for(int h=0; h<aryOrderWeek.size(); h++){    // 쿠폰 개수 만큼 for문 돈다
			...
			//System.out.println("aryCouponName.get(h): " + aryCouponName.get(h));
	%>
		<p><span id="cp3_<%=h%>" class="event_area" name="event_area" value="<%=s_orderWeek%>" data="<%=aryPer.get(h)%>" atype="<%=aryW.get(h)%>" usegoods="<%=aryUseGoods.get(h)%>"   uselimitcnt="<%=useLimitCnt.get(h)%>" uselimitprice="<%=useLimitPrice.get(h)%>" ><%=aryCouponName.get(h)%></span></p>
	<%
	}
...
function forCoupon3(){
   ...
   // $("event_area").hide();// 모든 사용가능한 쿠폰 보여주기
    ...
    for(var r=0; r<orderWeekSize; r++){	
		$("#cp3_" + r).hide();
        var cp_value = $("#cp3_" + r).attr('value');
        var UseGoods = $("#cp3_" + r).attr('usegoods');
 		var uselimitcnt = $("#cp3_" + r).attr('uselimitcnt');
		var uselimitprice = $("#cp3_" + r).attr('uselimitprice');
		...
		if( uselimitcnt <= buyQty && uselimitprice <= goodsTotalPrice ){	
			...
			if(UseGoods == 1){
				if(DevlType == "0002"){    // 택배
					//$("#cp3_" + r).show();
					//쿠폰계산식
					if($("#cp3_" + r).attr('atype') == "W"){        //선택할인
						if(optIsDevlWeek && optGubun1 != "02"){
							var CouponeSalePrice = Number($("#cp3_" + r).attr('data'));
