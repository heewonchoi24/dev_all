...
<% if("0301590".equals(groupCode)){ %>	
			<div class="opt_select_group">
				...
				<label for="opt_buy_weekend"><span></span><strong class="f16">주말 식단(토,일 포함)</strong></label>
...
<% } %>
		...
		<%
			if(bagCnt < 1){
				%>
				<input type="checkbox" id="opt_buy_bag" name="buy_bag" checked="checked"/>
				<%
			}else if(bagCnt > 0){
				%>
				<input type="checkbox" id="opt_buy_bag" name="buy_bag"<%="Y".equals((String)cartMap.get("BUY_BAG_YN")) ? " checked" : ""%>/>
				<%
			}
		%>
