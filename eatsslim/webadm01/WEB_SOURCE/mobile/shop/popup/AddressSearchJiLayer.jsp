								...
								<tr>
									<td colspan='2' style='text-align:center'>
										<a href="javascript:void(0);" class="btn btn_dgray huge square" style="max-width: 100px;" onclick="pgSearch(1);">검색</a>
										 &nbsp; &nbsp;  
										<a href="javascript:void(0);" class="btn btn_gray huge square" style="max-width: 100px;" onclick="fnAddressClosePop('close');">닫기</a>
									</td>
								</tr>
							</tbody>
						</table>
...
function fnSetAddrJi(trNum, zip, addr, addr2, delvType1, delvType2, gmno) {
	if (delvType1 == 'X' && delvType2 == 'X') {
		...
		if ($("#dtype").val() == '0001') {
			$("#rcv_zipcode").val(zip);
			$("#rcv_addr1").val(addr);
			$("#rcv_addr_bcode").val(gmno);
		} else {
			$("#tag_zipcode").val(zip);
			$("#tag_addr1").val(addr);
		}

		fnAddressClosePop('select');
	}
}
