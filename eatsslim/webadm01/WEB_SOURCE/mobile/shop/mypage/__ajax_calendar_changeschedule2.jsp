...
if(productTotalCnt > 0){
	...
	if (rs.next()) {
		startDate = rs.getString(1); //패턴변경 시작일
	}
	rs.close();
}
else{
...
<div id="changeDateCal" class="expansioncss">
    <div class="inner">
        <header class="pop_header"><h1>배송일 변경 (2/3)</h1></header>
        <div class="pop_content">
            ...
            <div class="tip_scheduler">
            	<p class="moveText"><span>변경하려는 패턴을 선택한 후, 변경을 시작하려는 일자를 선택해 주세요.</span></p>
            </div>
...
%>
            ...
            <div class="bottom_btn_area btn_group">
                <button type="button" class="btn btn_white square half back" onclick="calendarPop.addCont({url : '__ajax_calendar_changeschedule.jsp?orderNum=<%=orderNum%>&goodsId=<%=subNum%>&groupCode=<%=gubunCode%>&goods=<%=goods%>', direc : 'prev'})">뒤로가기</button>
                <button type="button" class="btn btn_dgray square half" onclick="updPtDate();">선택완료</button>
            </div>
            </form>
        </div>
    </div>
...
%>
    $('#pop_scheduler2').datepicker({
        ...
        onSelect: function onSelect(fd, date, picker) {
        	...
        	if(date.length == 1){
        		console.log(fd);
	            var currentDate = date[0].getDate();
	            var currentDay = date[0].getDay();
	            var total = currentDate + Number(dateGap);
	            $("#tar_start_date").val(fd);
	            $("#tar_end_date").val(fd);

	      / *   	if(currentDay + Number(dateGap) >= 6){
	   	        	//console.log(Math.floor((currentDay + Number(dateGap)) / 5));  //주말이 몇번끼었나 체크

	   	        	$(".datepicker--cell-day[data-date="+total+"]").trigger("click");
	        	}else{
		        	$(".datepicker--cell-day[data-date="+total+"]").trigger("click");
	        	} * /
        		...
        		//console.log("fd2=" + fd);
        		var arrFd = fd.split(",");
	            $("#tar_start_date").val(arrFd[0]);
	            $("#tar_end_date").val(arrFd[1]);
        	}
            */
	...
	function(data) {
		...
		$(data).find("result").each(function() {
			if ($(this).text() == "success") {
				...
				calendarPop.addCont({url : '__ajax_calendar_changeschedule3.jsp?'+param})
