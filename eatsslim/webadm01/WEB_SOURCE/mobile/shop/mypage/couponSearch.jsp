...
<div id="wrap">
	...
	<div id="content" class="oldClass">
		...
		<div id="couponSearch">
			<div class="topBox">
				<p>오프라인 쿠폰을 발급받으신 분은 <br/>이곳에서 쿠폰을 등록한 다음 사용하세요.</p>
				<div class="ipt_group">
			    	<input type="text" name="off_coupon_num" id="off_coupon_num" class="ipt" maxlength="20" />
			    </div>
			    <button type="button" class="btn btn_gray square" onclick="setCoupon();">쿠폰 등록하기</button>
			</div>
			<div class="listArea">
				<div class="title">사용가능한 쿠폰 <span><%=intTotalCnt%></span></div>
				<ul class="couponList">
					<!-- <li>
						<div class="titleHead">
							<span class="couponImg">20%</span>하계 20% 특별쿠폰
						</div>
						<div class="info">
							<p><span>유효기간:</span> 2017.06.13 ~ 2017.06.30</p>
							<p><span>쿠폰코드:</span> ET170613143924001</p>
							<p><span>발급날짜:</span> 2017. 06. 13</p>
							<p class="use">20,000원 이상 구매시 사용 가능</p>
							<p class="use">전체상품에 사용 가능</p>
						</div>
					</li>
					<li>
						<div class="titleHead">
							<span class="couponImg">5000원</span>하계 20% 특별쿠폰
						</div>
						<div class="info">
							<p><span>유효기간:</span> 2017.06.13 ~ 2017.06.30</p>
							<p><span>쿠폰코드:</span> ET170613143924001</p>
...
if (intTotalCnt > 0) {
	while (rs.next()) {
		...
		orderWeeks			= rs.getString("ORDERWEEK");
			...
			if (useLimitCnt > 0) {
				useLimitTxt			+= "<p class='use'>"+ nf.format(useLimitCnt) + "개 이상 구매시 사용 가능</p>\n";
			} else {
				useLimitTxt			= "";
			}
			if (useLimitPrice > 0) {
				useLimitTxt			+= "<p class='use'>"+ nf.format(useLimitPrice) + "원 이상 구매시 사용 가능</p>\n";
			} else {
				useLimitTxt			= "";
			}
			useGoods			= rs.getString("USE_GOODS");
			if (useGoods.equals("01")) {
				useGoodsTxt			= "<p class='use'>전체 상품에 사용 가능</p>\n";
				orderWeeks = orderWeeks.substring(0,orderWeeks.length() -1);
				orderWeeksTxt = orderWeeks + "(주) 사용가능\n";				
			} else if (useGoods.equals("03")) {
				useGoodsTxt			= "<p class='use'>일배상품 주문시 사용 가능</p>\n";
				orderWeeks = orderWeeks.substring(0,orderWeeks.length() -1);
				orderWeeksTxt = orderWeeks + "(주) 사용가능\n";		
			} else if (useGoods.equals("04")) {
				useGoodsTxt			= "<p class='use'>택배상품 주문시 사용 가능</p>\n";
			} else {
				...
				while (rs1.next()) {
					useGoodsTxt		+= "<p class='use'> "+ rs1.getString("GROUP_NAME") + " 주문시 사용가능</p>\n";
				...
				if(chk == 1){
				orderWeeks = orderWeeks.substring(0,orderWeeks.length() -1);
				orderWeeksTxt = "일배상품 >> " + orderWeeks + "(주) 사용가능\n";		
				}
...
%>
					<li>
						...
						<div class="info">
							<p><span>유효기간:</span> <%=stdate%> ~ <%=ltdate%></p>
							<p><span>쿠폰코드:</span> <%=couponNum%></p>
							<p><span>발급날짜:</span> <%=instDate%></p>
							<p><span>주문기간:</span> <%=orderWeeksTxt%></p>
...
%>
			...
			<div class="guide">
				다운받은 쿠폰은 <span>주문/결제 > 쿠폰적용</span> 화면에서 적용하실 쿠폰항목을 선택하시면 사용이 가능합니다.
			</div>
		</div>
	</div>
	<!-- End Content -->
