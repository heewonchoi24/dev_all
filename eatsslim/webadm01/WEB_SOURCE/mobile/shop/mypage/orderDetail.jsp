...
<div id="wrap" class="expansioncss">
	...
	<div id="container">
		<div class="container_inner">
			...
			query		+= "	COUPON_PRICE, DEVL_PRICE, PAY_TYPE, PG_CARDNUM, PG_FINANCENAME, PG_ACCOUNTNUM, ORDER_STATE,PG_TID";
			...
			query		= "SELECT OG.ORDER_CNT, OG.DEVL_TYPE, OG.DEVL_DAY, OG.DEVL_WEEK, DATE_FORMAT(OG.DEVL_DATE, '%Y.%m.%d') WDATE, OG.PRICE, OG.BUY_BAG_YN, G.GUBUN1, G.GROUP_NAME, G.CART_IMG, G.GROUP_IMGM,  O.COUPON_PRICE, OG.ID, G.GROUP_CODE, DEVL_PRICE, EXP_PROMOTION";
			query		+= " FROM ESL_ORDER_GOODS OG, ESL_GOODS_GROUP G, ESL_ORDER O";
			query		+= " WHERE OG.GROUP_ID = G.ID";
			query		+= " AND OG.ORDER_NUM = O.ORDER_NUM";
			query		+= " AND OG.ORDER_NUM = '"+ orderNum +"'";
			...
			query		= "SELECT OG.ORDER_CNT, OG.DEVL_TYPE, OG.DEVL_DAY, OG.DEVL_WEEK, DATE_FORMAT(OG.DEVL_DATE, '%Y.%m.%d') WDATE, OG.PRICE, OG.BUY_BAG_YN, G.GUBUN1, G.GROUP_NAME, G.CART_IMG, G.GROUP_IMGM, O.COUPON_PRICE, DEVL_PRICE";
			query		+= " FROM ESL_ORDER_GOODS OG, ESL_GOODS_GROUP G, ESL_ORDER O";
			query		+= " WHERE OG.GROUP_ID = G.ID";
			query		+= " AND OG.ORDER_NUM = O.ORDER_NUM";
			query		+= " AND OG.ORDER_NUM = '"+ orderNum +"'";
			...
			%>
							...
							<div class="cartTotal">
								총 주문금액 <span><%=nf.format(tagPrice)%></span> 원
							</div>
						</div>
			<%
			}
								...
								<table>
									<tbody>
										...
										<tr>
											<th>영수증</th>
											<td class="right">
											<button type="button" class="btn btn_dgray square" style="width:100px" onclick="window.open('https://pgweb.uplus.co.kr:9091/MpFlowCtrl?eventDiv1=search&eventDiv2=getReceipt&TRANSACTIONID=<%=pgTid%>&SYSTEM=NEW&authResultParam=Y',
											'pop','resizable=no,scrollbars=yes,width=700,height=800');">영수증 출력</button>
											</td>
										</tr>
									</tbody>
								</table>
