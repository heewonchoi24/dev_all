...
if (request.getHeader("REFERER")==null) {
	...
	if("02".equals(gubun1) ){
		...
		query		+= " AND GOODS_ID = "+ subNum +" AND GROUP_CODE not in ( '0300668')";
	...
	*/
	...
	query		= "SELECT EODD.ID, STATE, GROUP_CODE, EODD.ORDER_CNT, EOG.DEVL_WEEKEND, GROUP_ID FROM ESL_ORDER_DEVL_DATE EODD, ESL_ORDER_GOODS EOG WHERE EODD.ORDER_NUM = '"+ orderNum +"'";
	query		+= " AND GOODS_ID = "+ subNum +" AND GROUP_CODE not in ( '0300668', '0301340' )";
	query		+= " AND EODD.DEVL_DATE BETWEEN '" + orgStartDate + "' AND '" + orgEndDate + "' AND EODD.order_num = EOG.ORDER_NUM";
	query		+= " ORDER BY EODD.DEVL_DATE ";
	...
	while (rs.next()) {
			...
			if (chkCnt == 0) {
				for (m=0;m<patternDays.length;m++) {
					if (Integer.parseInt(patternDays[m]) == cal.get(cal.DAY_OF_WEEK)) {
						...
						// 주말포함 주문인 경우
						if ( cal.get(cal.DAY_OF_WEEK) == 6 ) { //금요일인 경우
								
							weekCnt    = rs.getString("DEVL_WEEKEND");
							//nWeekCnt = Integer.parseInt(weekCnt);
							if ( weekCnt.equals("1")  ) {
								nWeekCnt = 2;
		...
		if ( bDays ) {
			...
			if (groupId == 110) {
				...
				if (rs1.next()) {
					query1		+= "	GROUP_CODE			= '"+ rs1.getString("SET_CODE") +"',";
				}
				rs1.close();
