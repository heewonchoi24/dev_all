...
<div id="changeDateCal" class="expansioncss">
    <div class="inner">
        <header class="pop_header"><h1>배송일 변경 (3/3)</h1></header>
        <div class="pop_content">
            ...
            <div class="bx_pattern confirm">
                <div class="bx_pattern_inner">
                    <div class="text_area">
                        <strong><%=orgSdate%></strong> 부터 <strong><%=orgEdate%></strong> 까지의<br> 배송일정을 <span><%=tarSdate%></span> 부터 <span><%=tarEdate%></span> <br/>까지의 배송일정으로 변경처리 합니다.
                    </div>
                </div>
            </div>
            <div class="bottom_btn_area btn_group">
<%--                 <button type="button" class="btn btn_white square half back" onclick="calendarPop.addCont({url : '__ajax_calendar_changeschedule2.jsp?orderNum=<%=orderNum%>&goodsId=<%=subNum%>&groupCode=<%=gubunCode%>&goods=<%=goods%>', direc : 'prev'})">뒤로가기</button> --%>
