...
<%
	//-- 일일배송
	if(cartType1Ct > 0){
		query		= "SELECT C.ID, C.GROUP_ID, G.GROUP_CODE, C.BUY_QTY, C.DEVL_TYPE, C.DEVL_DAY, C.DEVL_WEEK,";
							...
							<li>
								<div class="inner">
									...
									<!--div class="cartbottom">
										<div class="price"><%=nf.format(payPrice)%>원</div>
									</div-->
								</div>
<%
			if (buyBagYn.equals("Y")) {
...
%>
					<div class="orderBox">
						...
						<ul class="cartList">
<%
		query		= "SELECT G.GROUP_CODE, C.ID, C.GROUP_ID, C.BUY_QTY, C.DEVL_TYPE, C.DEVL_DAY, C.DEVL_WEEK, DATE_FORMAT(C.DEVL_DATE, '%Y.%m.%d') WDATE, C.PRICE, C.BUY_BAG_YN, G.GUBUN1, G.GROUP_NAME, G.CART_IMG, G.GROUP_IMGM";
		query		+= " FROM ESL_CART C, ESL_GOODS_GROUP G";
...
<%} else {	// 150 프로틴밀		%>
								...
								query2		= " SELECT IFNULL(MAX(ID),0) AS ID FROM ESL_CART WHERE MEMBER_ID = '"+eslMemberId+"' AND GROUP_ID = '"+groupId+"' AND CART_TYPE='C' ";
								try {
