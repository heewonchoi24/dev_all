...
%>
							...
							%>
								<select name="coupon_code_<%=groupCode%>" id="coupon_code_<%=groupCode%>" onchange="setPrdtCoupon('<%=groupCode%>')" class="coupon_val inp_st inp_st100p">
									<option value="0|0|0">선택</option>
                                    <%
                                        int paramPriceA = 0;
                                        int paramPriceB = 0;
                                        int paramH = 0;
											...
											if( useLimitCnt.get(h) <= buyQty && useLimitPrice.get(h) <= goodsTotalPrice ){
												...
												if(aryUseGoods.get(h) == 1){
													...
													if(devlType.equals("택배")){
														%>
														<option id="cp3_<%=h%>" data="<%=arySaleNum.get(h)%>" value="<%=arySaleNum.get(h)+"|"+arySalePrice.get(h)+"|"+couponId%>" <%if(h == maxIdx){out.print(" selected");}%> ><%=arySaleName.get(h) +"("+ nf.format(arySalePrice.get(h)) +"원 할인)"%></option>
														<%
								...
								<input type="hidden" id="coupon_price_<%=groupCode%>" class="coupon_price" value="<%=couponMaxPrice%>" />
							<% } else { %>
							쿠폰없음
							<% } %>
							</div>
							...
							<input type="hidden" name="coupon_price" id="coupon_fprice_<%=groupCode%>" class="fprice" value="<%=couponMaxPrice%>" />
...
%>
			...
			<div class="orderBox totalPriceArea">
				...
				<div class="totalPriceTable">
					<table>
						...
						<tbody>
							...
							<tr>
								<th>할인혜택</th>
								<td>(-) <span id="coupon_fprice_txt">0</span></td>
							</tr>
				...
				<div class="totalPrice">
					총 주문금액 <span id="tprice"><%=nf.format(cartTotalAmount)%></span>
				</div>
			</div>
					...
					<table>
						<tbody>
							...
							<tr>
								<th>출입시 비밀번호</th>
								<td>
									<input name="rcv_pass_yn" id="rcv_pass_yn1" type="radio" value="Y" checked="checked">
									<label for="rcv_pass_yn1"><span></span>있음</label>
									<input name="rcv_pass" id="rcv_pass" type="text" style="width:50px;">
									<input name="rcv_pass_yn" id="rcv_pass_yn2" type="radio" value="N" >
...
<% } %>
			...
			<div class="orderBox">
				...
				<div class="orderBtns">
					...
					<button type="button" class="btn btn_dgray square" onclick="mGoOrder();">결제하기</button>
				</div>
			</div>
		</div>
		<div id="zipcodSearchLayerArea" style="display:none;"></div>
...
<input type="hidden" name="LGD_CLOSEDATE" value="<%=pgCloseDate%>" />
<input type="hidden" name="LGD_DISPLAY_BUYEREMAIL" value="N" />
</form>
...
function fu_couponSelect(gcd, paramH, paramVal){
   $("#coupon_code_"+ gcd +" option[id=cp3_"+paramH+"]").attr("selected","selected");
   if($("#coupon_code_"+ gcd +" option:checked").val() == "0|0|0"){
		$("#coupon_code_"+ gcd +" option").text("쿠폰없음");
		$("#coupon_code_"+ gcd).css("pointer-events","none"); 
   }else{
	    $("#coupon_code_"+ gcd +" option[id=cp3_"+paramH+"]").text($("#coupon_code_"+ gcd +" option:checked").text());
   }
   setPrdtCoupon(gcd);
}
...
function getCouponPrice(gcd) {
	...
	$("#coupon_fprice_txt").text(commaSplit(couponTprice) + "원");
...
function fnAddressPop(val){
	...
	$("#zipcodSearchLayerArea").load("/mobile/shop/popup/AddressSearchJiLayer.jsp?ztype="+ val, function(event) {
		$("#zipcodSearchLayerArea").show();
	});
}

function fnAddressClosePop(flag){
	$("#shopOrder").show();
	
	$("#zipcodSearchLayerArea").hide();
	$("#zipcodSearchLayerArea").html("");
	
	$("input[name=rcv_addr2]").focus();
}
		...
		$.ajax({
			...
			success : function(data){
				if(data.code == "success"){
					...
					alert(data.data);
					location.href = '/mobile/shop/experience.jsp';
				}
			},
