...
if (mode.equals("getGroup")) {
	try {
		query		= "SELECT ID, GROUP_NAME, GROUP_PRICE, GROUP_INFO, OFFER_NOTICE, GROUP_CODE, GUBUN3 FROM ESL_GOODS_GROUP";
		query		+= " WHERE GUBUN1 = '02' AND GUBUN2 = ? AND USE_YN = 'Y' ORDER BY ID ASC";
		...
		while (rs.next()) {
			...
			if (tcnt == 0) {
				...
				rs2.close();
				pstmt2.close();
				data		+= "<group>"+ groupId +"|<![CDATA["+ groupName +"]]>|"+ price +"|<![CDATA["+ groupInfo +"]]>|<![CDATA["+ gubun3 +"]]>|<![CDATA["+saleTitle+"]]>|<![CDATA["+saleType+"]]>|<![CDATA["+salePrice+"]]></group>";
			} else {
				data		+= "<group>"+ groupId +"|<![CDATA["+ groupName +"]]>|"+ price +"|<![CDATA["+ groupInfo +"]]>|<![CDATA["+ gubun3 +"]]></group>";
			}
			tcnt++;
		}
