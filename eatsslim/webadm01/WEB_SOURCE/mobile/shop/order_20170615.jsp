...
query		= "SELECT C.GROUP_ID, C.BUY_QTY, C.DEVL_TYPE, C.DEVL_DAY, C.DEVL_WEEK, DATE_FORMAT(C.DEVL_DATE, '%Y.%m.%d') WDATE, C.PRICE, C.BUY_BAG_YN, G.GUBUN1, G.GUBUN2, G.GROUP_CODE, G.GROUP_NAME, G.CART_IMG, G.GROUP_IMGM";
...
//						goodsPrice	= price * buyQty;
		...
		%>
		<dl class="itemlist">
			<dt style="width:70%;"><%=groupName%>/<%=buyQty%>개
		<% if (gubun1.equals("01") || gubun1.equals("02") || rs.getString("DEVL_TYPE").equals("0001")) { %>
		...
		<dl class="itemlist redline">
			...
			<dd class="f16 font-orange" id="tprice"><%=nf.format(cartTotalAmount)%>원</dd>
		</dl>
		<div class="divider"></div>
		...
		<ul class="form-regist">
			...
			<li>
				...
				<input name="tag_hp3" type="number" style="width:60px;" value="<%=memHp3%>" required label="휴대폰 번호" maxlength="4">
			</li>
			<li>
				<label>전화번호</label>
				<div class="select-box">
