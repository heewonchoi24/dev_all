...
if (mode.equals("del")) {
	cartId		= request.getParameter("cartId") == null ? 0 : Integer.parseInt(request.getParameter("cartId"));
	...
	if (bagYn.equals("B")) {
		...
		if (purchaseCnt < 1) {
			...
			try {
				query		= "UPDATE "+ table +" SET PRICE=PRICE-" + defaultBagPrice + ", BUY_BAG_YN = 'N' WHERE MEMBER_ID = '"+ eslMemberId +"' AND ID = ?";
		...
		try {
			query		= "DELETE FROM "+ table +" WHERE MEMBER_ID = '"+ eslMemberId +"' AND ID = ?";
			pstmt		= conn.prepareStatement(query);
			pstmt.setInt(1, cartId);
			pstmt.executeUpdate();
	...
	query		= "DELETE FROM "+ table +" WHERE MEMBER_ID = '"+ eslMemberId +"' AND ID IN ("+ delIds +")";
