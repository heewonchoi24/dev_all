...
$(function () {
  ...
  if (!msie6) {
	...
	var quick_menu = $('#floatMenu, #left_banner');
        ...
        onAdd : function(devlWeek,devlDay,buyQty,type,cartId,devlType,buyBagYn,day,sumPriceWeek,groupName){
            ...
            TweenMax.to( clone, .1, {
                ...
                onComplete : function(){
                    ...
                    if(devlDay == "5"){
    					appendBx += '        <div class="goods_caption">占쏙옙~占쏙옙/ '+devlWeek+'占쏙옙 占싹븝옙</div>';

    				}else if(devlDay == "3"){
    					appendBx += '        <div class="goods_caption">占쏙옙/占쏙옙/占쏙옙 / '+devlWeek+'占쏙옙 占싹븝옙</div>';
    				}

                    appendBx += '        <div class="goods_foot">';
                    appendBx += '            <div class="goods_price">'+sumPriceWeek+'</div>';
                    appendBx += '			<a href="/shop/popup/__ajax_goods_set_options_select.jsp?lightbox[width]=500&lightbox[height]=530&groupName='+groupName+'&groupCode=&groupPrice=&groupPrice1='+sumPriceWeek+'&groupInfo1=&kalInfo=&totalPrice='+sumPriceWeek+'&groupInfo1=&cateCode=&devlType='+devlType+'&cartId='+cartId+'&paramType=list" class="lightbox goods_options">占심션븝옙占쏙옙</a>';
                    appendBx += '        </div>';
                    appendBx += '    </div>';
