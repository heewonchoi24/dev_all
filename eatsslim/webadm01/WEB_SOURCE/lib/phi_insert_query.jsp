...
/*		if (rcvPassYn.equals("Y")) {
	...
	*/
	...
	query		+= "	DATE_FORMAT(DEVL_DATE, '%Y-%m-%d') DEVL_DATE, PRICE, BUY_BAG_YN, SS_TYPE, EXP_PROMOTION";
	...
	while (rs.next()) {
		...
		promotion_	= ut.inject(rs.getString("EXP_PROMOTION"));// 체험단 프로모션
		if(!promotion_.equals("01")){
		price		= (gubun1.equals("01"))? rs.getInt("PRICE") / (( Integer.parseInt(devlDay) + Integer.parseInt(devlHoliday) ) * Integer.parseInt(devlWeek)) : rs.getInt("PRICE"); //-- 개별 단가로 설정
		}else{// 체험단 프로모션
...
*/
					...
					if (cal.get(cal.DAY_OF_WEEK) == 7 || cal.get(cal.DAY_OF_WEEK) == 1) { // 토, 일요일은 배송을 하지 않는다	
						...
						if (gubun1.equals("02")) {
							query2		= "SELECT GROUP_CODE, PRICE FROM ESL_PG_DELIVERY_SCHEDULE WHERE GUBUN2 = '"+ gubun2 +"'";
							if (gubun2.equals("27") || gubun2.equals("28") || gubun2.equals("29") || gubun2.equals("51")) {
...
*/
			...
			if (gubun2.equals("33")) {
			...
			} else if(groupCode.equals("0301844")){	// 150 프로틴밀

				query2		= " SELECT BUY_CNT, P_SET_CODE FROM ESL_CART_DELIVERY where ORDER_NUM = '" + orderNum + "'";
				try {
					rs2 = stmt2.executeQuery(query2);
				} catch(Exception e) {
					out.println(e+"=>"+query2);
				...
				while(rs2.next()){
					...
					orderCnt = rs2.getInt("BUY_CNT");
					groupCode = rs2.getString("P_SET_CODE");
					//System.out.println("========================");
