		...
		<ul id="dropline">
			...
			<li class="top<%if (request.getRequestURI().indexOf("/intro") >= 0) out.print(" current");%>"><a class="down" href="/intro/eatsslimStory.jsp"><span class="m02"></span>잇슬림 소개</a>
				<ul class="sub">
					...
					<li<%if (request.getRequestURI().indexOf("/intro/eatsslimCounsel") >= 0) out.print(" class=\"current\"");%>><a href="/intro/eatsslimCounsel.jsp">자문단 소개</a></li>
					<li<%if (request.getRequestURI().indexOf("/intro/eatsslimCkitchen") >= 0) out.print(" class=\"current\"");%>><a href="/intro/eatsslimCkitchen.jsp">센트럴 키친</a></li>
				</ul>
			</li>
			...
			<li class="top<%if (request.getRequestURI().indexOf("/event") >= 0) out.print(" current");%>"><a class="down" href="/event/currentEvent.jsp"><span class="m05"></span>이벤트</a>
				<ul class="sub floatRight">
					...
					<li<%if (request.getRequestURI().indexOf("/event/versus") >= 0) out.print(" class=\"current\"");%>><a href="/event/versus.jsp">다이어트 서바이벌1</a>
						<ul class="sub">
							<li<%if (request.getRequestURI().indexOf("/event/observe") >= 0) out.print(" class=\"current\"");%>><a href="/event/observe.jsp">최과장&amp;이대리 밀착감시</a></li>
							<li<%if (request.getRequestURI().indexOf("/event/badHabit") >= 0) out.print(" class=\"current\"");%>><a href="/event/eventView.jsp?id=17">나쁜 습관 응모</a></li>
							<li<%if (request.getRequestURI().indexOf("/event/versus") >= 0) out.print(" class=\"current\"");%>><a href="/event/versus.jsp">최과장VS이대리</a></li>
						</ul>
					</li>
				</ul>
			</li>
