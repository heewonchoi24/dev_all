			...
			<div class="one last col">
				<div class="orderSearch">
					<label for="select">주문</label>
						...
						query		+= " AND ((OG.ORDER_STATE > 0 AND OG.ORDER_STATE < 90) OR OG.ORDER_STATE = '911')";
							...
							<tbody>
								<tr>
									...
									<td class="fc-header-center">
										<span class="fc-button fc-button-prev fc-state-default fc-corner-left fc-corner-right" unselectable="on" style="-moz-user-select: none;" onclick="location.href='delichange_test.jsp?year=<%=preYear %>&month=<%=preMonth%>&ono=<%=orderNum%>&gid=<%=goodsId%>&gno=<%=groupCode%>&pdate=<%=payDate%>'">
...
function cngGoods() {
	...
	location.href = "delichange_test.jsp?ono="+ orderArr[0] +"&gno="+ orderArr[1] +"&pdate="+ orderArr[2] +"&gid="+ orderArr[3];
}
...
function nextMonth() {
	location.href='delichange_test.jsp?year=<%=nextYear %>&month=<%=nextMonth%>&ono=<%=orderNum%>&gid=<%=goodsId%>&gno=<%=groupCode%>&pdate=<%=payDate%>&week_day=' + $("input[name=week_day]:last").val() + "&sch_week=<%=week%>";
}
</script>
</body>
</html>