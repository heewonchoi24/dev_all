...
query		= "SELECT ORDER_NUM, DATE_FORMAT(ORDER_DATE, '%Y.%m.%d') ORDER_DATE, DATE_FORMAT(DATE_ADD(ORDER_DATE, INTERVAL 1 MONTH), '%Y%m%d') ORDER_DATE_TO, ORDER_NAME, PAY_TYPE, PAY_PRICE,";
...
<div id="wrap">
	...
	<div class="container">
		<div class="maintitle">
			<h1>주문배송 조회</h1>
			<div class="pageDepth">
				HOME &gt; 마이잇슬림 &gt; <strong>주문/배송</strong>
			</div>
			<div class="clear"></div>
		</div>
					...
					<table class="orderList" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						if (intTotalCnt > 0) {
							while (rs.next()) {
								...
								orderDateTo	= (rs.getString("ORDER_DATE_TO") == null)? "" : rs.getString("ORDER_DATE_TO");
								...
								query1		+= "		(SELECT MIN(DEVL_DATE) FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ orderNum +"' AND GUBUN_CODE = concat(G.GUBUN1,G.GUBUN2)) MIN_DAT, ";
								query1		+= "		(SELECT MAX(DEVL_DATE) FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ orderNum +"' AND GUBUN_CODE = concat(G.GUBUN1,G.GUBUN2)) MAX_DAT ";
								...
								while (rs1.next()) {
									...
									if ( devlType.equals("0001") ){
										if (orderState.equals("01")) {
											devlState			+= rs1.getString("MIN_DAT") + "~<br>" + rs1.getString("MAX_DAT") + "</div>";
										}
									} else {
										if (orderState.equals("01")) {
											devlState			+= "<a href=\"javascript:;\" onclick=\"window.open('http://www.doortodoor.co.kr/tracking/jsp/cmn/Tracking_new.jsp?QueryType=1&pOrderNo="+orderNum+"1&pFromDate="+orderDate.replace(".", "")+"&pToDate="+orderDateTo+"&pCustId=0010049837','pop','resizable=no,scrollbars=yes,toolbar=no,location=no,status=no,menubar=no,width=1013,height=600')\")>배송조회</a>";
										}
									}
									devlState		+= "</div>";
