...
query		= "SELECT ID, FAQ_TYPE, TITLE, HIT_CNT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE";
	...
	<div id="container" class="group">
		...
		<div id="incon">
			...
			<div id="location">
				...
				<p id="path"><img src="../images/common/layout/ico_home.gif" alt="홈" /> &gt; 상담/문의관리 &gt; <strong>FAQ</strong></p>
			</div>
			<!-- //location -->
			<!-- contents -->
			<div id="contents">
					...
					<table class="table01" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								<th scope="row">
									<span>
										<select style="width:80px;" name="sch_type" onchange="this.form.keyword.focus()">
											<option value="01"<%if(schType.equals("01")){out.print(" selected=\"selected\"");}%>>교환/환불</option>
											<option value="02"<%if(schType.equals("02")){out.print(" selected=\"selected\"");}%>>회원관련</option>
											<option value="03"<%if(schType.equals("03")){out.print(" selected=\"selected\"");}%>>결재관련</option>
											<option value="04"<%if(schType.equals("04")){out.print(" selected=\"selected\"");}%>>배송관련</option>
											<option value="09"<%if(schType.equals("09")){out.print(" selected=\"selected\"");}%>>기타</option>
										</select>
									</span>
								</th>
								<th scope="row">
				...
				<form name="frm_list" id="frm_list" method="post" action="faq_db.jsp">
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									faqId		= rs.getInt("ID");
									faqType		= ut.getFaqType(rs.getString("FAQ_TYPE"));
									title		= rs.getString("TITLE");
									hitCnt		= rs.getInt("HIT_CNT");
									instDate	= rs.getString("WDATE");
							%>
							<tr>
								<td><input type="checkbox" class="selectable" value="<%=faqId%>" /></td>
								<td><%=curNum%></td>
								<td><%=faqType%></td>
								<td><%=title%></td>
								<td><%=hitCnt%></td>
								<td><%=instDate%></td>
								<td>
									<a href="faq_edit.jsp?id=<%=faqId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
				...
				<div class="btn_style1">
					...
					<p class="right_btn">
						<a href="faq_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div>
