...
query		+= "	ANSWER_YN, CONTENT, TIME_FORMAT(TIMEDIFF(IFNULL(ANSWER_DATE, NOW()), INST_DATE), '%H') INTERVAL_HOUR, TIME_FORMAT(TIMEDIFF(IFNULL(ANSWER_DATE, NOW()), INST_DATE), '%i') INTERVAL_MIN";
					...
					<table class="table01" border="1" cellspacing="0">
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									<input type="text" name="reg_stdate" id="reg_stdate" class="input1" maxlength="10" readonly="readonly" value="<%=regStdate%>"/>
									~
									<input type="text" name="reg_ltdate" id="reg_ltdate" class="input1" maxlength="10" readonly="readonly" value="<%=regLtdate%>"/>
								</td>
							</tr>
						</tbody>
					</table>
					<p class="btn_center">
						<a href="javascript:;" onclick="chkSearch();"><img src="../images/common/btn/btn_search.gif" alt="검색" /></a>
					</p>			
						...
						<tbody>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									ansInterval	= rs.getString("INTERVAL_HOUR") +":"+ rs.getString("INTERVAL_MIN");
							%>
							<tr>
...
$(document).ready(function() {
	...
	$('#reg_stdate,#reg_ltdate').datepick({ 
...
function customRange1(dates) {
	if (this.id == 'reg_stdate') { 
        $('#reg_ltdate').datepick('option', 'minDate', dates[0] || null); 
    } else { 
        $('#reg_stdate').datepick('option', 'maxDate', dates[0] || null); 
    }
}
