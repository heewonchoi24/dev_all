...
String[] orderWeek	= request.getParameterValues("order_week");	// ¡÷πÆ±‚∞£
String orderWeeks = "";	
...
if (mode != null) {	
	if (mode.equals("ins")) {
		...
		query		+= "	USE_LIMIT_PRICE, USE_GOODS, MAX_COUPON_CNT, INST_ID, INST_IP, INST_DATE, ORDERWEEK)";
		query		+= " VALUES";
		query		+= "	(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)";
		...
		if (useGoods.equals("02")) {
			...
			query		+= "	(COUPON_ID, GROUP_CODE, GROUP_NAME)";
			query		+= " VALUES";
			query		+= "	("+ couponId +", '0300578', 'πÎ∑±Ω∫Ω¶¿Ã≈©')";
			try {
				stmt.executeUpdate(query);
			} catch (Exception e) {
				out.println(e+"=>"+query);
