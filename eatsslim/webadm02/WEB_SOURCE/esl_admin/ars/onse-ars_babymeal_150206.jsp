...
<%
	...
	try{ 
		 ...
		 strBufSql.append("SELECT CUSTOMER_NUM, INDEX_NO FROM pulmuone.dbo.ITF_MEMBER_INFO_FNC('9002300000', '129884') A WHERE A.HPHONE_TYPE = '"+strPhoneNum1+"' and A.HPHONE_FIRST = '"+strPhoneNum2+"' and A.HPHONE_SECOND = '"+strPhoneNum3+"'");
		 ...
		 if(customer_num == 0){
			 ...
			 // 등록		 
			try{
				String fileUrl = "https://www.pulmuone.co.kr/sso/sso_site_used_itf_proc.asp?";
				String fileParam = "RETURN_TYPE=XML&CMD=U&REG_SITE_NO=9002300000&ACCESS_KEY=129884&CUSTOMER_NUM="+customer_num+"&INDEX_NO="+index_no+"&SITE_NO=9002300000&SMS_YN=N";
				URL url = new URL(fileUrl+fileParam);
			...
			if(con_mssql!=null)try{con_mssql.close();}catch(SQLException sqe){} 
	} 

	if(sucess_flg == true){ 
		String query	= "";

		query		= "UPDATE NBM_MEMBER SET SMS_YN = 'N' WHERE CUSTOMER_NUM = '"+ customer_num +"'";
		try {
			stmt_phi2.executeUpdate(query);
		} catch(Exception e) {
			out.println(e+"=>"+query);
			if(true)return;
		}
