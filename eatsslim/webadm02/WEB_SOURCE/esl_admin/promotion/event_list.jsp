...
query		= "SELECT ID, GUBUN, EVENT_TYPE, TITLE, OPEN_YN, STDATE, LTDATE, ANC_DATE, HIT_CNT, DATE_FORMAT(INST_DATE, '%Y.%m.%d') WDATE, WINNER";
		...
		<div id="incon">
			...
			<div id="contents">
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<th scope="col"><span>웹/모바일</span></th>
								<th scope="col"><span>공개여부</span></th>
								<th scope="col"><span>구분</span></th>
								<th scope="col"><span>제목</span></th>
								<th scope="col"><span>이벤트기간</span></th>
								<th scope="col"><span>발표일자</span></th>
								<th scope="col"><span>등록일</span></th>
								<th scope="col"><span>당첨자</span></th>
							...
							%>
							<tr>
								<td><input type="checkbox" class="selectable" value="<%=eventId%>" /></td>
								<td><%=curNum%></td>
								<td><%=gubun%></td>
								<td><%=openYn%></td>
								<td><%=ut.getEventType(eventType)%></td>
								<td><%=ut.cutString(50, title, "..")%></td>
								<td><%=stdate%>~<%=ltdate%></td>
								<td><%=ancDate%></td>
								<td><%=instDate%></td>
								<td>
									<a href="event_winner_write.jsp?id=<%=eventId + param%>" class="function_btn"><span><%=winnerTxt%></span></a>
								</td>
								<td>
									<a href="event_edit.jsp?id=<%=eventId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
							...
							%>
							<tr>
								<td colspan="11">등록된 이벤트가 없습니다.</td>
							</tr>
							<%
							}
							%>
				...
				<div class="btn_style1">
					...
					<p class="right_btn">
						<a href="event_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div>
