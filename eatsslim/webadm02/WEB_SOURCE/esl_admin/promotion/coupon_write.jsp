					...
					<table class="tableView sch_member" border="1" cellspacing="0">
						...
						<tbody>
							...
							<tr>
								<th scope="row">
									<span>
										<select style="width:80px;height:24px;" name="field" onchange="this.form.keyword.focus()">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									신규회원
									<input type="radio" name="vendor" value="07" />
									로그인
								</td>
							...
							<tr>
								...
								<td>
									...
									상품판매가격의
									<input type="text" name="sale_price1" id="sale_price1" value="0" style="width:60px;height:24px;" maxlength="3" dir="rtl" onblur="this.value=this.value.replace(/[^0-9]/g,'');" />
									% 할인
									<input type="radio" name="sale_type" value="W" />
									상품판매가격의
									<input type="text" name="sale_price2" id="sale_price2" value="0" style="width:60px;height:24px;" maxlength="7" dir="rtl" onkeyup="onlyNum(this);" />
									원 할인
								</td>
								<th scope="row"><span>주문기간</span></th>
								<td>
									<input type="checkbox" id="order_week" name="order_week" value="1" checked="checked"/>1주
									<input type="checkbox" id="order_week" name="order_week" value="2" />2주
									<input type="checkbox" id="order_week" name="order_week" value="4" />4주
									<input type="checkbox" id="order_week" name="order_week" value="8" />8주
								</td>
							</tr>
							<!--tr>
							...
							<tr>
								...
								<td colspan="3">
									구매최소수량
									<input type="text" name="use_limit_cnt" id="use_limit_cnt" style="width:60px;height:24px;" maxlength="2" dir="rtl" value="0" onkeyup="onlyNum(this);" />
									개 /
									상품판매가격의
									<input type="text" name="use_limit_price" id="use_limit_price" style="width:60px;height:24px;" maxlength="7" dir="rtl" value="0" onkeyup="onlyNum(this);" />
									원 이상
								</td>
							</tr>
							<tr>
							...
							<tr id="offline" class="hidden">
								<th scope="row"><span>오프라인용<br />쿠폰타입</span></th>
								<td colspan="3">
									<input type="radio" name="rand_type" value="S" checked="checked" />
									수량제한
									<input type="radio" name="rand_type" value="G" />
									무제한사용
								</td>
							</tr>
...
$(document).ready(function() {
	...
	$("input[name=use_goods]").click(function() {
		...
		if (useGoods == '04') {
			alert("택배 상품 전체 사용가능 쿠폰을 발급할 경우 주문기간은 적용되지 않습니다."); 
...
function chkWrite() {
	...
	var useGoods	= $("input[name=use_goods]:checked").val();
	if (useGoods == 01 || useGoods == 03) {// 전체, 특정, 일배
		if($("input[name=order_week]:checked").length == 0){// 주문기간에 체크가 안되어 있으면
			alert("주문기간을 선택해주세요.");
			return;
		}
	}
	...
	if(chk == 1){
		alert("선택한 일배 상품에 대한 주문기간을 선택해주세요.");
		return;
	}
