...
query		= "SELECT ID, VERSUS, TITLE, DAY, WEEKDAY, DATE_FORMAT(INST_DATE, '%Y.%m.%d') INST_DATE";
	...
	<div id="container" class="group">
		...
		<div id="incon">
			...
			<div id="contents">
								...
								<th scope="row">
									<span>
										<select style="width:80px;" name="field" onchange="this.form.keyword.focus()">
											<option value="TITLE"<%if(field.equals("MEMBER_ID")){out.print(" selected=\"selected\"");}%>>제목</option>
											<option value="VERSUS"<%if(field.equals("VERSUS")){out.print(" selected=\"selected\"");}%>>일기등록자</option>
											<option value="CONTENT"<%if(field.equals("MEMBER_ID")){out.print(" selected=\"selected\"");}%>>내용</option>
										</select>
									</span>
								</th>
								<td>
									<input type="text" class="input1" style="width:200px;" maxlength="30" name="keyword" value="<%=keyword%>" onfocus="this.select()"/>
									일기등록자 검색 (1: 최과장, 2: 이대리)
								</td>
							</tr>
						</tbody>
					</table>
				...
				<form name="frm_list" id="frm_list" method="post" action="diary_db.jsp" enctype="multipart/form-data">
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<th scope="col"><span>등록자</span></th>
								<th scope="col"><span>제목</span></th>
								<th scope="col"><span>차수</span></th>
							...
							%>
							<tr>
								<td><input type="checkbox" class="selectable" value="<%=diaryId%>" /></td>
								<td><%=curNum%></td>
								<td><%=versus%></td>
								<td><%=title%></td>
								<td><%=day%>일차(<%=weekday%>)</td>
								<td><%=instDate%></td>
								<td>
									<a href="diary_edit.jsp?id=<%=diaryId + param%>" class="function_btn"><span>수정</span></a>
								</td>
							</tr>
							<%
									curNum--;
							...
							%>
							<tr>
								<td colspan="7">등록된 다이어트 일기가 없습니다.</td>
							</tr>
							<%
							}
							%>
				...
				<div class="btn_style1">
					...
					<p class="right_btn">
						<a href="diary_write.jsp" class="function_btn"><span>등록</span></a>
					</p>
				</div>
