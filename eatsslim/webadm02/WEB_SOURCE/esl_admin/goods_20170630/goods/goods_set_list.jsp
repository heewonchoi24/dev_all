...
query2		= "SELECT a.ID, a.CATEGORY_ID, a.SET_CODE, a.SET_NAME, a.SET_PRICE, a.THUMB_IMG, a.USE_YN, DATE_FORMAT(a.INST_DATE, '%Y.%m.%d') WDATE, b.CATE_NAME";
							...
							<tr>
								...
								<td>
									<select name="sch_cate">
										...
										<%
										query3		= "SELECT ID, CATE_NAME, CATE_CODE FROM ESL_GOODS_CATEGORY WHERE OPEN_YN = 'Y'";
										query3		+= " ORDER BY CATE_CODE DESC";
										...
										%>
										<option value="<%=cateId%>"<%if(schCate == cateId){out.print(" selected=\"selected\"");}%>><%=cateName+"("+rs1.getString("CATE_CODE")+")"%></option>
										<%
										}
								...
								<td>
									<select style="width:80px;" name="field" onchange="this.form.keyword.focus()">
										<option value="SET_NAME"<%if(field.equals("SET_NAME")){out.print(" selected=\"selected\"");}%>>메뉴명</option>
										<option value="SET_CODE"<%if(field.equals("SET_CODE")){out.print(" selected=\"selected\"");}%>>메뉴코드</option>
									</select>
					...
					<table class="table02" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<th scope="col"><span>메뉴코드</span></th>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									if (!thumbImg.equals("")) {
										imgUrl		= "<img src=\""+ webUploadDir +"goods/"+ thumbImg +"\" width=\"115\" height=\"65\" />";
									} else {
										imgUrl		= "&nbsp;";
									}
									useYn		= (rs.getString("USE_YN").equals("Y"))? "사용" : "미사용";
