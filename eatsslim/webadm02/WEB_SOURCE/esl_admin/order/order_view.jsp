...
query		+= "	ORDER_NAME, ifnull((SELECT CH_NAME FROM ESL_CHANNEL WHERE CH_CODE = SHOP_TYPE ), SHOP_TYPE) as SHOPTYPE, OUT_ORDER_NUM";
...
if (rs.next()) {
	...
	shopType		= rs.getString("SHOPTYPE");
...
<body>
	...
	<div class="popup_wrap">
		...
		<div style="padding: 20px 0; text-align: center;">
			<a href="javascript:void(0);" onclick="popup('order_all_cancel.jsp?ordno=<%=orderNum%>&pay_type=<%=payType%>',900, 300,'order_all_cancel');" class="function_btn"><span>전체취소</span></a>
		</div>
				...
				if (tcnt > 0) {
					...
					while (rs.next()) {
						...
						if (buyBagYn.equals("Y")) {
							...
							int orderBagCnt		= 0;
							query1		= "SELECT ORDER_CNT";
							query1		+= " FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ orderNum +"'";
							query1		+= " AND GOODS_ID = '"+ goodsId +"'";
							query1		+= " AND GROUP_CODE = '0300668'";
							try {
								rs1			= stmt.executeQuery(query1);
							} catch(Exception e) {
								out.println(e+"=>"+query1);
							...
							if (rs1.next()) {
								orderBagCnt			= rs1.getInt("ORDER_CNT");
							}

							rs1.close();
							bagPrice	+= defaultBagPrice * orderBagCnt;
						}
