		...
		<table class="table01 mt_5" border="1" cellspacing="0">
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="3">
						<!--a href="javascript:;" onclick="window.open('http://www.doortodoor.co.kr/tracking/jsp/cmn/Tracking_new.jsp?QueryType=1&pOrderNo=<%=orderNum%>1&pFromDate=<%=orderDateFrom%>&pToDate=<%=orderDateTo%>&pCustId=0010049837','pop','resizable=no,scrollbars=yes,toolbar=no,location=no,status=no,menubar=no,width=600,height=600')" class="ui-btn ui-mini ui-btn-up-b floatright">배송조회하기</a-->
						<a href="javascript:;" onclick="window.open('https://www.lotteglogis.com/open/tracking?invno=<%=invNo%>','pop','resizable=no,scrollbars=yes,toolbar=no,location=no,status=no,menubar=no,width=800,height=600')" class="ui-btn ui-mini ui-btn-up-b floatright">배송조회하기</a>
					</td>
				</tr>	
		...
		<!-------------------------------------->
					...
					<table class="table02 mt_5" border="1" cellspacing="0">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" name="order_cnt" id="order_cnt" value="1" onkeyup="this.value=this.value.replace(/[^0-9-]/g,'');" dir="rtl" style="width:40px;" maxlength="2" /></td>
			...
			<div class="tab_layout" id="tablmenu3">
				...
				<table class="table02 mt_5" border="1" cellspacing="0">
					...
					<tbody>
						<%
						query		= "SELECT ID, DEVL_DATE, WEEKDAY(DEVL_DATE) WEEKDAY, ORDER_CNT, PRICE, STATE, GROUP_CODE,";
						query		+= "	(SELECT GROUP_NAME FROM ESL_GOODS_GROUP WHERE GROUP_CODE = '"+ gubunCode +"') GROUP_NAME";
						query		+= " FROM ESL_ORDER_DEVL_DATE";
						query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
						query		+= " AND GOODS_ID = "+ subNum;
						...
						while (rs.next()) {
							...
							if (Integer.parseInt(state) < 90) {
								if (today.compareTo(rs.getString("DEVL_DATE")) >= 0) {
									devlState		= "배송완료";
								} else {
									devlState		= "배송준비중";
								}
						...
						<tr>
							...
							<td>
								<%=groupName+"("+prdtSetName+")"%>
								<%if (state.equals("01")) {%>
								<input type="button" value="취소" onclick="refundBag('<%=payType%>', <%=rs.getInt("ORDER_CNT") * rs.getInt("PRICE")%>, <%=rs.getInt("ID")%>);" id="refundBag_<%=rs.getInt("ID")%>" />
