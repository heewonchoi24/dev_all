		...
		if (totalCnt > 0) {
			while (rs.next()) {
				...
				if (Integer.parseInt(rs.getString("SHOP_CD")) > 52 || !shopOrderNum.equals("")) {
					query1		= "SELECT IF (OG.DEVL_TYPE = '0001', IF(OG.PRICE * OG.ORDER_CNT < O.COUPON_PRICE, 0, ((OG.PRICE * OG.ORDER_CNT) - O.COUPON_PRICE) / (OG.DEVL_DAY * OG.DEVL_WEEK * OG.ORDER_CNT)), IF(OG.PRICE * OG.ORDER_CNT < O.COUPON_PRICE, 0, ((OG.PRICE * OG.ORDER_CNT) - O.COUPON_PRICE) / OG.ORDER_CNT)) AS PAY_PRICE,";
					...
					query1		= "SELECT IF (DEVL_TYPE = '0001', IF(PRICE * ORDER_CNT < COUPON_PRICE, 0, ((PRICE * ORDER_CNT) - COUPON_PRICE) / (DEVL_DAY * DEVL_WEEK * ORDER_CNT)), IF(PRICE * ORDER_CNT < COUPON_PRICE, 0, ((PRICE * ORDER_CNT) - COUPON_PRICE) / ORDER_CNT)) AS PAY_PRICE,";
				...
				query1		+= " WHERE ORDER_NUM = '"+ orderNum +"' AND GOODS_ID = '"+ orderSub +"'";
				query1		+= " AND STATE < 90 AND GROUP_CODE not in ('0300668', '0301340')";
				query1		+= " AND PRICE > 0";
				if (etype.equals("1") || etype.equals("3")) {
					query1		+= " AND ORDER_DATE >= '"+ stdate +"'";
					query1		+= " AND ORDER_DATE <= '"+ ltdate +"'";
