...
where			= " WHERE not (ORDER_STATE = '00' AND PAY_TYPE = '10') ";
...
query		+= "	ORDER_STATE, ORDER_ENV, SHOP_TYPE, ifnull((SELECT CH_NAME FROM ESL_CHANNEL WHERE CH_CODE = SHOP_TYPE ), SHOP_TYPE) as SHOPTYPE";
								...
								<td>
									<span>
										<select name="sch_shop_type">
											...
											<%
												query1	= "SELECT CH_CODE, CH_NAME FROM ESL_CHANNEL ORDER BY ID";
												try {
												...
												while (rs1.next()) {
											%>
											<option value="<%=rs1.getString("CH_CODE")%>"<%if(schShopType.equals("51")){out.print(" selected=\"selected\"");}%>><%=rs1.getString("CH_NAME")%></option>
											<%
												}
											%>											
										</select>
						...
						<tbody>
							...
							if (intTotalCnt > 0) {
								while (rs.next()) {
									...
									query1		+= "		DEVL_DAY, DEVL_WEEK, EXP_PROMOTION";
									...
									i		= 0;
									while (rs1.next()) {
										promotion	= ut.inject(rs1.getString("EXP_PROMOTION"));
										if (i > 0) {
									...
									query1		+= " FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ orderNum +"' AND STATE < 91 ";
									try {
							...
							<tr>
								...
								%>
								<td><%=productName%><span style="color:red">(체험단)</span></td>
								<%
								}else{
								%>
								<td><%=productName%></td>
