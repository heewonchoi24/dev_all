...
//System.out.println(eslAdminId + "   " + eslAdminMenu);
...
if (instId == null || instId.equals("")) {
	...
	if (chkCnt > 0) {
		...
		if (orderCnt > 0) {
			...
			if ( gubunCode.equals("0301590") ) {
				query1		= "SELECT SET_CODE FROM ESL_GOODS_CATEGORY_SCHEDULE WHERE CATEGORY_ID = 20 AND DEVL_DATE  = '"+ devlDate + "'";
				...
				if (rs1.next()) {
					groupCode		= rs1.getString("SET_CODE");
	...
	//===========================================================================================
	//-- START : 테이스티 세트 - 배송상품코드,가격 순서를 저장하고 그 만큼 위에서 선언한 groupCodeList에 상품 코드와 가격을 담아준다.
	int intOrderCnt = 0;
    query        = " SELECT COUNT(*), ORDER_DATE  FROM ESL_ORDER_DEVL_DATE WHERE ORDER_NUM = '"+ orderNum +"' GROUP BY ORDER_DATE ";
	try {
		rs		= stmt.executeQuery(query);
	} catch(Exception e) {
		out.println(e+"=>"+query);
	...
	if (rs.next()) {
		intOrderCnt = rs.getInt(1); //총 레코드 수		
	}
	
	// 20190308 테이스티 세트 리뉴얼: 기존 주문건 일정 변경 시 기존 코드로 진행
	tastyOrderDate = Integer.parseInt(rs.getString("ORDER_DATE"));
    if(tastyOrderDate > 20190308) tastyTable = "ESL_PG_DELIVERY_SCHEDULE";
    else tastyTable = "ESL_PG_DELIVERY_SCHEDULE_TASTY_OLD";
    
	ArrayList<String[]> groupCodeList = new ArrayList<String[]>();// 상품 메뉴코드 및 가격 담을 LIST 선언
	if( gubunCode.equals("0301737") ){
		for(int k=0; k<intOrderCnt; k++){
            query        = " SELECT GROUP_CODE, PRICE FROM " + tastyTable + " WHERE GUBUN2 = 51 ";
			try {
				rs		= stmt.executeQuery(query);
				while(rs.next()){
					groupCodeList.add( new String[]{rs.getString("GROUP_CODE"), rs.getString("PRICE")} );
				}
	...
	//===========================================================================================
		...
		if (gubunCode.substring(0, 2).equals("02")) {
			...
			query		+= " AND GOODS_ID = "+ subNum +" AND GROUP_CODE not in ( '0300668', '0301340', '0070817' )";
			query		+= " AND PRICE > 0 ";
		...
		if (!orgDevlDate.equals(devlDate)) {
			...
			query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 배송일 변경 ("+ orgDevlDate +" => "+ devlDate +") ', ' || 담당자: "+ eslAdminName +" ', NOW())";
...
<%
		...
		if (orgOrderCnt != orderCnt) {
			query		=  "UPDATE ESL_ORDER SET ";
			query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 수량 변경 ("+ devlDate +" : "+ orgOrderCnt +" => "+ orderCnt +") ', ' || 담당자: "+ eslAdminName +" ', NOW()) ";
			query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
			try {
				stmt.executeUpdate(query);
			} catch(Exception e) {
		...
		//===========================================================================================
		//-- 테이스티 세트 - 배송상품코드,가격 순서를 저장하고 그 만큼 위에서 선언한 groupCodeList에 상품 코드와 가격을 담아준다.
		if( gubunCode.equals("0301737") ){
			...
			try {
				rs		= stmt.executeQuery(query);
				int codeCt = 0;
				//System.out.println("프로그램 다이어트 재정렬 시작 :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");
				while(rs.next()){
					try{
				...
				//System.out.println("프로그램 다이어트 재정렬 종료 :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");
			} catch(Exception e) {
				out.println(e+"=>"+query);
				if(true)return;
			}
...
<%
	...
	query		= "UPDATE ESL_ORDER SET ";
	query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 일괄변경: ', '패턴일(" + devlDatePattern + ")', ' || 담당자: "+ eslAdminName +" ', NOW())";
	query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
	try {
		stmt1.executeUpdate(query);
	} catch(Exception e) {
	...
	if (chkCnt > 0) {
		...
		query		+= " AND GROUP_CODE not in ( '0300668', '0301340', '0070817' )";
		...
		if (rs.next()) {
			...
			if (gubunCode.equals("0331")) {
				...
				rs1.close();
			} else if ( gubunCode.equals("0301590") ) {
				query2		= "SELECT SET_CODE FROM ESL_GOODS_CATEGORY_SCHEDULE WHERE CATEGORY_ID = 20 AND DEVL_DATE  = '"+  stdate + "'";
...
<%
			...
			query1		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 증정 반영("+ stdate +") ', ' || 담당자: "+ eslAdminName +" ', NOW())";
...
<%
			query1		= "UPDATE ESL_ORDER SET ";
			query1		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 배송, 수량 추가("+ stdate +", "+ orderCnt +") ', ' || 담당자: "+ eslAdminName +" ', NOW())";
			query1		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
			try {
				stmt1.executeUpdate(query1);
			} catch(Exception e) {
	...
	*/
	...
	while (rs.next()) {
		...
		if ( bDays ) {
			...
			if ( gubunCode.equals("0301590") ) {
				query2		= "SELECT SET_CODE FROM ESL_GOODS_CATEGORY_SCHEDULE WHERE CATEGORY_ID = 20 AND DEVL_DATE  = '"+  dt.format(cal.getTime()) + "'";
				//System.out.println(query2);
				try {
					rs1 = stmt1.executeQuery(query2);
				} catch(Exception e) {
		...
		for (int n=1;n<intTotalCnt-intCurCnt+1;n++) {
			...
			if (rs.next()) {
				...
				if (gubunCode.equals("0300668")) {
					...
					insPrice		= defaultBagPrice;
					insPayPrice		= defaultBagPrice;
				} else if( gubunCode.equals("0301737") ){
					groupCode		= groupCodeList.get(codeCt2)[0];
					insGubunCode	= "0301737";
					insPrice		= Integer.parseInt(groupCodeList.get(codeCt2)[1]);
					insPayPrice		= Integer.parseInt(groupCodeList.get(codeCt2)[1]);
					codeCt2++;
				} else {
					groupCode		= rs.getString("GROUP_CODE");
					insGubunCode	= rs.getString("GUBUN_CODE");
...
<%
	...
	query		= "UPDATE ESL_ORDER SET ";
	query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 요일패턴 일괄변경: ', '패턴일(" + devlDatePattern + ")', ' || 담당자: "+ eslAdminName +" ', NOW())";
	query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
	try {
		stmt1.executeUpdate(query);
	} catch(Exception e) {
...
<%
	...
	String addrBcode	= ut.inject(request.getParameter("rcv_addr_bcode"));
	...
	if ((!zipcode.equals("") && zipcode != null) || (!rcvRequest.equals("") && rcvRequest != null)) {
		...
		*/
		...
		if (!addrBcode.equals("") && addrBcode != null) {
			query		= "SELECT JISA_CD FROM CM_ZIP_NEW_M WHERE GM_NO= '"+ addrBcode +"' AND ROWNUM = 1 ";
			try {
				rs_bm		= stmt_bm.executeQuery(query);
			} catch(Exception e) {
				out.println(e+"=>"+query);
			...
			query		= "SELECT JISA_CD, GM_NO FROM CM_ZIP_NEW_M WHERE (ZIP= '"+ zipcode +"' OR POST = '"+ zipcode +"')  AND JISA_CD IS NOT NULL AND ROWNUM = 1 ";
			try {
				rs_bm		= stmt_bm.executeQuery(query);
			} catch(Exception e) {
				out.println(e+"=>"+query);
			...
			if (rs_bm.next()) {
				rcvPartner		= rs_bm.getString("JISA_CD");
				addrBcode     = rs_bm.getString("GM_NO");
			}
			rs_bm.close();			
...
<%
		...
		query		= "UPDATE ESL_ORDER SET ";
		query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 배송정보변경("+ stdateAddr +" ~ "+ ltdateAddr +") ', ' || 담당자: "+ eslAdminName +" ', NOW())";
		query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
		try {
			stmt.executeUpdate(query);
		} catch(Exception e) {
...
<%
		...
		query		= "UPDATE ESL_ORDER SET ";
		query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 배송점정보변경("+ stdateAgcy +" ~ "+ ltdateAgcy +") ', ' || 담당자: "+ eslAdminName +" ', NOW())";
		query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
		try {
			stmt.executeUpdate(query);
		} catch(Exception e) {
	...
	if (Integer.parseInt(shopType) > 52) {
		...
		query1		= "SELECT IF (DEVL_TYPE = '0001', (COUPON_PRICE) / ( (DEVL_DAY + DEVL_WEEKEND) * DEVL_WEEK * ORDER_CNT), (COUPON_PRICE) / ORDER_CNT) AS DC_PRICE";
	...
	query1		+= " AND STATE = '01' AND GROUP_CODE not in ( '0300668', '0301340', '0070817' )";
...
<%
		...
		query		= "UPDATE ESL_ORDER SET ";
		query		+= "	ORDER_LOG = CONCAT(ORDER_LOG,'\nBOS 수령자정보변경() ', ' || 담당자: "+ eslAdminName +" ', NOW())";
		query		+= " WHERE ORDER_NUM = '"+ orderNum +"'";
		try {
			stmt.executeUpdate(query);
		} catch(Exception e) {
