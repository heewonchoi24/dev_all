			...
			if ($(this).text() == "success") {
				...
				$(data).find("address").each(function() {
					...
					if (zipcodeArr[0] == "nodata") {
						$("#AddressList").prepend('<tr><td colspan="2">검색된 배송가능지역이 없습니다.</td></tr>');
					} else {
						$("#AddressList").prepend('<tr><td>'+ zipcodeArr[0] +"</td><td><a href=\"javascript:zipcode('"+ zipcodeArr[0] +"','"+ zipcodeArr[1] +"')\">"+ zipcodeArr[1] +'</a></td></tr>');
					}
				});
			} else {
...
function zipcode(zip,addr){
	$("#tag_zipcode1", opener.document).val(zip.substring(0,3));
	$("#tag_zipcode2", opener.document).val(zip.substring(3,6));
