...
<% 
		...
		if (rs.next()) {
			...
			if (Integer.parseInt(cDate) >= 20140303 && Integer.parseInt(cDate) <= 20140331) {
				query		= "SELECT COUNT(ID) FROM ESL_COUPON_MEMBER";
				query		+= " WHERE MEMBER_ID = '"+ memberId +"' AND COUPON_ID = 404";
				try {
					rs		= stmt.executeQuery(query);
				} catch(Exception e) {
					out.println(e+"=>"+query);
				...
				if (couponCnt > 0) {
					...
					query		+= " VALUES (404,'"+ couponNum +"','"+ memberId +"','N',NOW())";
					try {
