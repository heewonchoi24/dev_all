...
query		= "SELECT DISTINCT SIDO FROM PHIBABY.V_ZIPCODE";
query		+= " WHERE DLVYN = 'Y' AND DLVTYPE= '0001' AND DLVPTNCD = '01' ORDER BY SIDO";
try {
		...
		<form name="frmZipcode" id="frmZipcode" method="get">
						...
						<div class="select-box">
							<select name="gugun" id="gugun">
								...
								if (mode.equals("schZipcode")) {
									query		= "SELECT DISTINCT GUGUN FROM PHIBABY.V_ZIPCODE ";
									query		+= " WHERE DLVYN = 'Y' AND DLVTYPE= '0001' AND DLVPTNCD = '01'";
									query		+= " AND SIDO LIKE '%"+ schSido +"%'";
			...
			<ul class="deli-result">
				...
				if (mode.equals("schZipcode")) {
					...
					query		+= " FROM PHIBABY.V_PARTNER P, PHIBABY.V_ZIPCODE Z";
...
function showZipcode() {
	if (!$.trim($("#sido").val())) {
		...
		f.action		= "delivery_test.jsp";
		f.target		= "";
		f.submit();
	}
}
</script>
<iframe id="ifrmHidden" name="ifrmHidden" src="about:blank" style="width:100%;height:200px;"></iframe>
</body>
</html>
