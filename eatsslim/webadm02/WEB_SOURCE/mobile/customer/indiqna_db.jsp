...
*/
	if ( mode.equals("ins") ) {
		...
		query		+= "	(MEMBER_ID, COUNSEL_TYPE, NAME, HP, EMAIL, TITLE, CONTENT, INST_IP, INST_DATE, ECS_CATE1, ECS_CATE2, ECS_CATE3)";
		query		+= " VALUES";
		query		+= "	(?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?)";
		...
		pstmt.setString(9, ecs_cate1);
		pstmt.setString(10, ecs_cate2);
		pstmt.setString(11, ecs_cate3);
		pstmt.executeUpdate();
		try {
		...
		query		+= "	'"+ email +"', '"+ ecs_cate1 +"', '"+ ecs_cate2 +"', '"+ ecs_cate3 +"', '"+ content +"', '"+ cDate +"', 'N', 'N', '잇슬림', '953')";
		try {
