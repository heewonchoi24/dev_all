...
function goAjax(){
	document.form.action = "/mngLevelObj/mngLevelObjectDetailAjax.do";
	document.form.submit();
}
</script>
   ...
   <section id="container" class="sub">
       <div class="container_inner">
			...
			<div class="box-info mb30">
				...
				<div class="cont">
					<ul class="lst-dots">
						<li>기관명을 클릭하시면 이의신청 정보를 확인하실 수 있습니다.</li>
									...
									<c:set var="tmpInsttCd" value="${i.INSTT_CD }" />						
										...
										<c:forEach var="j" items="${mngLevelInsttEvlList }" varStatus="status">
											<c:if test="${j.INSTT_CD == tmpInsttCd}">
												<td headers="">
													<c:choose>
														<c:when test="${j.EXCP_PERM_YN == 'Y'}"><span class="ico_state i_none"><em class="hidden">해당없음</em></span></c:when>
														<c:when test="${empty j.STATUS}"></c:when>
				                						<c:when test="${j.FOBJCT_RESN ne ''}"><span class="ico_state i_obj"><em class="hidden">이의신청</em></span></c:when>
				                					</c:choose>
												</td>
											</c:if>
										</c:forEach>
