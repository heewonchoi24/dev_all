      ...
      <div class="inr-c">
        <nav class="lst_right clearfix">
          <ul class="lst customer clearfix">
          	<c:choose>
          		<c:when test="${sessionScope.userInfo.authorId == null || sessionScope.userInfo.authorId == ''}"> 
	          		<li><a href="#"><span class="login_top IcoAfter">로그인</span></a></li>
	            	<li><a href="#"><span class="login_join IcoAfter">회원가입</span></a></li>
          		</c:when>
          		<c:otherwise>
...
<script type="text/javascript">
  ...
  function viewThread(url, seq, menuId){
		$("#sUrl").val(url);
		$("#seq").val(seq);
		$("#sMenuId").val(menuId);
		$("#headerForm").attr({
